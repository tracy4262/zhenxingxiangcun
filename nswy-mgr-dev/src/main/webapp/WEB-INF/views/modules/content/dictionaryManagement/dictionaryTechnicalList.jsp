...
<form:form id="searchForm"  modelAttribute="dictionaryTechnical"  action="${ctx}/nswy/dictionaryTechnical/control" method="post" class="breadcrumb form-search">
...
<script type="text/template" id="treeTableTpl">
	<tr id="{{row.id}}" pId="{{pid}}">
		...
		<shiro:hasPermission name="sys:menu:edit">
			<td nowrap>
				<a href="${ctx}/nswy/dictionaryTechnical/addPid?pid={{row.pid}}&level={{row.level}}">增加同级</a>
				<a href="${ctx}/nswy/dictionaryTechnical/add?id={{row.id}}&level={{row.level}}">增加下级</a>
				<a href="${ctx}/nswy/dictionaryTechnical/update?id={{row.id}}">修改</a>
				<a href="${ctx}/nswy/dictionaryTechnical/delete?id={{row.id}}" onclick="return confirmx('确定删除吗？', this.href)">删除</a>
			</td>
		</shiro:hasPermission>
	</tr>
</script>
