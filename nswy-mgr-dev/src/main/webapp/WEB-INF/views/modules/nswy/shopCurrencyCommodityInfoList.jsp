...
<body>
<form:form id="searchForm"  modelAttribute="shopCurrencyCommityInfo"  action="${ctx}/nswy/shopCurrencyCommodity/findShopCurrencyCommodityInfoList" method="post" class="breadcrumb form-search">
    <input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
    <input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
    通用商品名：<input type="text" name="commonProductName" maxlength="8" value="${commonProductName}"/>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
    ...
    <c:forEach items="${page.list}" var="item">
        <tr>
            ...
            <td title="${item.commodityAlias}" width="50px">
                <c:if test="${fn:length(item.commodityAlias) > 20 }">
                    ${fn:substring(item.commodityAlias,0,20)}...
                </c:if>
                <c:if test="${fn:length(item.commodityAlias) < 21 }">
                    ${item.commodityAlias}
                </c:if>
            </td>
            ...
            <td>
                <c:if test="${item.approveStatus eq '0'}">
                    <a href="${ctx}/nswy/shopCurrencyCommodity/shopCurrencyCommodityInfoFrom?shopId=${item.shopId}">审核</a>
                </c:if>
            </td>
        </tr>
    </c:forEach>
...
<script type="text/javascript">
    ...
    /*function adopt(id,approveStatus) {
        ...
        if (confirmStatus) {
            $.ajax({
                type : 'POST',
                url : '${ctx}/nswy/shopCurrencyCommodity/currencyAdopt',
                data : {
                    shopId : id,
