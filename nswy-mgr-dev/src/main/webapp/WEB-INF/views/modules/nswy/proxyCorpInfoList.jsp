...
<body>
<form:form id="searchForm"  modelAttribute="proxyCropInfo" action="${ctx}/nswy/getProxyCorpInfolist" method="post" class="breadcrumb form-search">
	<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
	<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
	<sys:tableSort id="orderBy" name="orderBy" value="${page.orderBy}" callback="page();"/>
	<label>被代理单位名称：</label><form:input path="corpName" htmlEscape="false" maxlength="50" class="input-medium"/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="item">
			<tr>
				<td title="${item.corpName}" width="50px">
				...
				<td>
                    ...
                    <a href="${ctx}/nswy/deleteProxyCorpInfo?proxyCorpInfoId=${item.proxyCorpInfoId}" onclick="return confirm('确认要删除该代理吗？')">删除</a>
					<c:if test="${item.approveStatus eq 0}">
						<a href="${ctx}/nswy/proxyCorpInfoAuditFrom?proxyCorpInfoId=${item.proxyCorpInfoId}">审核</a>
					</c:if>
				</td>
			</tr>
		</c:forEach>
