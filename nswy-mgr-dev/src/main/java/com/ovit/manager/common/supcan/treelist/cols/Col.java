package com.ovit.manager.common.supcan.treelist.cols;
...
public class Col {
	 ...
	 * 通常是允许，但是当列不可编辑时(包括隐藏时)，将以supcan.xml中的 <pasteAbleWhenUnEditAble> 的设定为准;
	 */
	@XStreamAsAttribute
	...
	/**
	 * 列宽 整数 - 绝对宽度 (单位:像素数); 百分比 - 窗口宽度的百分比, 如：20%; 为小于1的分数 - 比例因子，用于分配剩余的宽度,
	 * 如0.2; fitHeader 或 header - 自动伸展到能使表头标题能够完整显示;
	 */
	@XStreamAsAttribute
	...
	/**
	 * 下拉列(droplis, droptreelist)单元格的文字显示方式 0 - 仅显示文字部分; 1 - 仅显示键值(即key)部分; 2 -
	 * 键值+" - "+文字;
	 */
	@XStreamAsAttribute
