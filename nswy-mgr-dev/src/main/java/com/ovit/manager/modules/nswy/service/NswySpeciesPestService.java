...
public class NswySpeciesPestService extends CrudService<ExpertApplyDao, ExpertApply> {

    @Autowired
    private ExpertApplyDao ExpertApplyDao;

    public Page<Map<String,Object>> listSpeciesPest(Map<String, Object> params) {
        String pageNo;
        String pageSize;
        String fname;
        ...
        String url = Global.getConfig("api.url")+"/wiki/api/wiki/listSpeciesPest";
    ...
    @Transactional()
    public void updateSpeciesPest(Map<String,Object> params){
        List<Map<String,Object>> list;
        ...
        if (params.get("auditstatus") != null){
            try {
                ...
                url = Global.getConfig("api.url")+"/wiki/api/wiki/updateSpeciesPestAudit";
                ...
                if(auditstatus.equals("2")||auditstatus.equals("0")){
                    url = Global.getConfig("api.url")+"/wiki/api/wiki/findByPestName";
                    ...
                    Map<String,Object> wiki = ExpertApplyDao.findPestList(map);
                    if(MapUtils.isEmpty(wiki)){
                        ExpertApplyDao.savePestLibraryInfo(map);
                    }
               ...
               if(auditstatus.equals("0")){//更新待审核
                    ...
                    url = Global.getConfig("api.url")+"/wiki/api/wiki/updatePestPassAudit";
                   ...
                   map.put("image", MapUtils.getString(list.get(0),"fimagesrc"));
                   map.put("classId",MapUtils.getString(list.get(0),"speciesid"));
                   ExpertApplyDao.updatePestLibraryInfo(map);
                }else if(auditstatus.equals("3")){//删除待审核
                    url = Global.getConfig("api.url")+"/wiki/api/wiki/listSpeciesPest";
                    StringBuffer jsonBody = new StringBuffer();
