<template>
    ...
    -->
	<div class="publish">
		<Modal v-model="options.showPublish" width="750" heigth="700px" @on-cancel="cancel('mydynamic')" :title="'发布'+options.title" :mask-closable="false" :styles="{top: '20px'}" @on-visible-change="refresh">
			<!-- <Tabs @on-click="showMydynamic">
				<Tab-pane :label="item.columnName" v-for="(item,index) in options.columnSetting" v-show="indexpane <= 2"> -->
					<!-- <Tabs type="card"  @on-click="changeLabel">
						<Tab-pane :label="info.name" v-for="(info,index) in smallPaneData" :key="index"  :name="info.name">
						</Tab-pane>
			...
			<div class="tc" v-show=" indexpane >= 3"><img src="../img/developing.png"/>
				<p>开发中,敬请期待···！</p>
			</div>

			<RadioGroup v-model="mydynamic.leixin" type="button" @on-change="check(mydynamic)" class="mb20">
				<Radio :disabled="!wenzhang" label="文章">
				</Radio>
				<Radio :disabled="!tuce" label="图册">
				</Radio>
				<Radio :disabled="!yinping" label="音频">
				</Radio>
				<Radio  :disabled="!shiping" label="视频">
				</Radio>
				<Radio :disabled="!tushu" label="图书">
				</Radio>
				<Radio :disabled="!duanxin" label="短信">
				</Radio>
				<!--<Radio label="微博">
				</Radio>-->
			</RadioGroup>
			...
			<div style="margin-top: 10px;">
				<vui-steps :data="stepDataSec" :active="current" v-show="process === 4 ||process === 5 ||process === 6 ||process === 7"></vui-steps>
			</div>
			...
			:mydynamic="mydynamic"
			:indexpane="indexpane"
            @bookparent="bookparent"></book>

			<Form ref="mydynamic" :model="mydynamic" style="margin-top: 27px;" :rules="ruleInline" label-position="right" :label-width="100" v-show="process === 1 && indexpane <= 2">
...
<!--                <FormItem label="发布人" prop="author">
				...
				<FormItem label="摘要" prop="summary">
					<Input type="textarea" v-model="mydynamic.summary" placeholder="摘要" :maxlength="200">
					</Input>
				</FormItem>
				...
				<FormItem label="视频"  v-show="video" prop="music">
					<!--<dvideo  ></dvideo>-->
					<upload-video @saveDescribe="saveDescribe"></upload-video>
				</FormItem>
				<FormItem label="图册" prop="content" v-show="picture">
					<!--<upload @imgs="getCon" :imgNum="5" :buttonCon="buttonCon" :type='type'></upload>-->
					<!-- <publish-upload  @on-imgs="getCon" ></publish-upload> -->
					 <vui-upload
                        @on-getPictureList="getCon"
                        :hint="'图片大小小于2MB，最多上传 50 张'"
                        :total="50"
                        :size="[100,100]"
                    ></vui-upload>
				</FormItem>
				<FormItem label="音频"   v-show="audios" prop="video">
					<!--<aplayer @videoResult="saveDescribe" ></aplayer>-->
					<upload-music @videoResult="saveDescribe" ></upload-music>
				</FormItem>
				<FormItem label="短信内容" prop="content" v-show="msm">
					<Input type="textarea" v-model="mydynamic.content" placeholder="最多200个字" :maxlength="length">
					</Input>
				</FormItem>
				...
				<FormItem label="详细内容" prop="content" v-show="con">
						<quil-editor @quilCon="getDetail" :uploadId="options.uploadId" :paneType="mydynamic.leixin"></quil-editor>
				</FormItem>
				<FormItem label="封面图片">
					<!--<upload @imgs="getCon" :imgNum="5" :buttonCon="buttonCon" :type='type'></upload>-->
					<!-- <upload-cover  @on-cover="getCover" ></upload-cover> -->
					 <vui-upload
                        @on-getPictureList="getCover"
			...
			<Form ref="pubstep2" :model="mydynamic" style="margin-top: 27px;" :rules="ruleInline2" label-position="right" :label-width="100" v-show="process === 2">
				<Row>
					<Col>
				...
				<Row>
					...
					<FormItem label="章节" prop="chapter" v-if="options.knowledgeShow">
						<Input type="text" v-model="mydynamic.chapter" placeholder="章节">
						</Input>
					</FormItem>
					</Col>
				</Row>
				...
				<template v-if="options.policyShow">
					<Row>
						...
						<Form-item label="发文单位" prop="company">
							<Input type="text" v-model="mydynamic.company" placeholder="发文单位">
							</Input>
						</Form-item>
						</Col>
					</Row>
					<Row>
						...
						<Form-item label="颁布时间" prop="promulgation">
							<DatePicker type="date"  v-model="mydynamic.promulgation" :transfer='transfer'>
							</DatePicker>
						</Form-item>
						</Col>
						<Col>
						<Form-item label="发文字号" prop="font">
							<Input type="text" v-model="mydynamic.font" placeholder="发文字号">
							</Input>
						</Form-item>
						</Col>
					</Row>
				...
				<Row>
					...
					<FormItem label="行业分类" prop="industry">
						<Input v-model="mydynamic.industryName" readonly @on-focus="handleFilterModal('tradeFilter')" />
					</FormItem>
					</Col>
				</Row>
				<label id="ivu-form-item-label-custom" class="ivu-form-item-label" style="width: 100px;">适用区域</label>
				<FormItem :label-width="0">
					<Row>
						<!--<Col span="6">
							<Select transfer v-model="country" @on-change="getCountry">
									<Option label="中国" value="China"></Option>
									<Option label="不限" value="unlimited"></Option>
							</Select>
						</Col>-->
						<Col >
			...
			<Form ref="pubstep3" style="margin-top: 27px;" :model="mydynamic"  label-position="right" :label-width="100" v-show="process === 3 ">
			...
			<div slot="footer" style="text-algin:center" v-show="indexpane <= 2">
				<div style="text-algin:center">
					<Button type="primary" shape="circle" @click.native="preStep" style="width:100px;margin-right: 20px;" v-if="process !=1 && process !=4">上一步</Button>
					...
					<Button type="primary" shape="circle" @click.native="next('pubstep2')" style="width:100px;margin-right: 20px;" v-if="process ===2" >下一步</Button>
					<Button type="primary" shape="circle" @click.native="publishSubmit('pubstep3')" style="width:100px" v-if="process === 3">发布</Button>
					<Button type="primary" shape="circle" @click.native="next('showbook')" style="width:100px;margin-right: 20px;" v-if="process ===4">下一步</Button>
					<Button type="primary" shape="circle" @click.native="next('showbook2')" style="width:100px;margin-right: 20px;" v-if="process ===5">下一步</Button>
					<Button type="primary" shape="circle" @click.native="next('showbook3')" style="width:100px;margin-right: 20px;" v-if="process === 6">下一步</Button>
					<Button type="primary" shape="circle" @click.native="publishSubmit('showbook4')" style="width:100px" v-if="process === 7">发布</Button>

				</div>
			</div>

			<!-- 相关行业tradeFilter -->
			<vui-filter ref="tradeFilter" :num="1" :simple="true" :pageShow="true" :total="total2" :pageCur="pageCur2" :resultDatas="tradeResultDatas" @on-search="handleTradeSearch" @on-page-change="handleTradePageChange" @on-get-result="handleGetTradeResult"/>
			<!-- 相关物种 -->
			<vui-filter ref="speciFilter" :num="5" :cols="2" :pageShow="true" :total="total" :pageCur="pageCur" :classifyDatas="speciClassifyDatas" :resultDatas="speciResultDatas" :load-data="loadSpeciDatas" @on-search="handleSpeciSearch" @on-get-classify="handleGetSpeciClassify" @on-get-result="handleGetSpeciResult" @on-page-change="handleSpeciPageChange" />

			<!-- 相关服务 -->
			<vui-filter ref="serviceFilter" :num="5" :simple="true" :resultDatas="serviceResultDatas" @on-search="handleServiceSearch" @on-get-result="handleGetServiceResult" />
			<!-- 相关商品名 -->
...
<script>
	...
	import quilEditor from "./vuequilEditor";
	import quilWeibo from "./vuequilEditorWeibo";
    ...
    import vuiSteps from './vui-steps'
    import book from './book.vue'
    import vuiUpload from './vui-upload'
	export default {
		name: "dany",
		components: {
			quilEditor,
		...
		created() {
			...
			this.$api.post('/member/login/findCurrentUser', {
				...
				console.log('res1233333', res)
				if (res.data.displayName) {
					this.mydynamic.author = res.data.displayName
				}
			})
			// this.mydynamic.author = JSON.parse(sessionStorage.getItem(sessionStorage.getItem('key'))).displayName
			// 取地址
		...
		methods: {
                     ...
                     axios.post(`${this.$url.uploadBase64}/imgbase64/upload`,
                         ...
                         .then(response => {
                             if(response.data.code==="0"){
								 ...
								 console.log('裁剪图片--->',this.cover_photo)
                             }else{
                                 this.$Message.error(response.data.msg)
                             }
                         })
			...
			showMydynamic(index = 0) {
				...
				if('政策' === columnName){
					this.showPol()
                    this.mydynamic.category = '030201'
				}else if('知识' === columnName){
					this.showLed()
                    this.mydynamic.category = '040101'
				}else if('动态' === columnName){
					this.showDany()
                    this.mydynamic.category = '020107'
				}
				// 获取自定义标签
				this.$api.get('/member/columnSettings/findColumnByUserSettings?account=' + this.loginuserinfo.loginAccount).then(response => {
					...
					if (response.code === 200) {
						...
						this.allSmallPaneData.forEach(element => {
							if (element.typeName === columnName) {
            ...
            handleTabsDel () {
				this.$Modal.confirm({
					...
					content: '确定删除该标签？',
					onOk: () => {
						let columnName = this.options.columnSetting[this.indexpane].columnName
						this.allSmallPaneData.forEach(element => {
							if (element.typeName === columnName) {
			...
			showLed() { //发布知识
				...
				this.mydynamic.contentType = 'knowLege'
				this.options.categorys = [{
						label: '科普',
						value: '040101'
					},
			...
			publishSubmit(name) {
						...
						if(leixin == "图书"){
                            ...
                            api.post('/member' + this.options.realUrl, {
                                promulgation: this.moment(this.mydynamic.promulgation).format('YYYY-MM-DD HH:mm:ss'),
                                ...
                                book_print_time:this.moment(this.$refs.bookData.mydynamic.book_print_time).format('YYYY-MM-DD HH:mm:ss'),
                                book_word_count:this.$refs.bookData.mydynamic.book_word_count,
                                book_pub_date:this.moment(this.$refs.bookData.mydynamic.book_pub_date).format('YYYY-MM-DD HH:mm:ss'),
			...
			next(name) {
				if('mydynamic' === name || 'showbook' === name ){
					if('微博' === this.mydynamic.leixin){
						this.$refs[name].validate(valid=> {
                            ...
                            this.tuce = '',
                            this.duanxin = ''
						if (this.mydynamic.videoUrl==='' || this.mydynamic.videoUrl.length <= 0){
				...
				} else if('showbook2' === name || 'showbook3' === name || 'showbook4' === name){
                    let flag = this.$refs.bookData.yanzheng(name);
                    if(flag){
                        if(this.current === 4){
                            this.current=0
			...
			saveDescribe(val){
				if(val.length){
					val.forEach(e=>{
						e.column=this.options.columnSetting[this.indexpane].columnName
					})
