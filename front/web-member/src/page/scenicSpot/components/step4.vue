...
<script>
    export default {
        ...
        methods:{
                    ...
                    if (valid) {
                        this.$api.post('/member/fishing/updateFishingService',this.data).then(response=>{
                            if(response.code == 200){
                                ...
                                // this.$router.push('/scenicSpotAddService/step5')
                                // this.$router.push('/scenicSpot/service')
                                this.$router.push('/scenicSpotAddService/step5?id=' + this.data.id)
                            }
                        })  
                    } else {
            ...
            // 上一步
            handleBack () {
                this.$router.push('/scenicSpotAddService/step2?id=' + this.data.id)
            },
        }
    }
</script>