<template>
    <div class="order-contents-list">
        ...
        <Row style="background:#f7f7f7" type="flex" justify="center" align="middle">
            ...
            <Col span="3"> <div class="pd10 tc">门票名称</div> </Col>
            <!-- <Col span="2"> <div class="pd10 tc">预定餐桌</div> </Col> -->
            <Col span="2"> <div class="pd10 tc">游玩时间</div> </Col>
            <Col span="3"> <div class="pd10 tc">价格</div> </Col>
                    ...
                    <Row type="flex" justify="center" align="middle" >
                      ...
                      <Col span="3">
                        <div class="pt10 tc" v-if="data.checkType === '0'">
                            <p v-for="(item, index) in data.setMeal" class="pb10">{{item.ticketName}}</p>
                        </div>
                      ...
                      <Col span="2">
                        <div class="pd10 tc">{{moment(data.date).format('YYYY-MM-DD')}}</div>
                      </Col>
                      <Col span="3">
                        <div class="pt10" v-if="data.checkType === '0'">
                            <div v-for="(item, index) in data.setMeal">
                                <div class="pb10 tc">￥{{!item.discountPrice ? parseFloat(item.ticketPrice).toFixed(2) : parseFloat(item.discountPrice).toFixed(2)}}</div>
                            </div>
                        </div>
                        <div class="pt10" v-if="data.checkType === '1'">
                            <div v-for="(item, index) in data.setMeal[0].productList">
                                <div class="pb10 tc">￥{{!item.total ? parseFloat(0).toFixed(2) : parseFloat(item.total).toFixed(2)}}</div>
                            </div>
                        </div>
                      </Col>
                      <Col span="2">
                        <div class="pd10 tc" v-if="data.discountPrice">
                            <p style="text-decoration: line-through;">￥{{!data.price ? parseFloat(0).toFixed(2) : parseFloat(data.price).toFixed(2)}}</p>
                            <p>￥{{!data.discountPrice ? parseFloat(0).toFixed(2) : parseFloat(data.discountPrice).toFixed(2)}}</p>
                            <!-- ￥{{!data.discountPrice ? parseFloat(0).toFixed(2) : parseFloat(data.discountPrice).toFixed(2)}} -->
                        </div>
                        <div class="pd10 tc" v-else>
                            ￥{{!data.price ? parseFloat(0).toFixed(2) : parseFloat(data.price).toFixed(2)}}
                        </div>
                      </Col>
                      <Col span="3">
