    ...
    <div class="follow-filter">
      <Row type="flex" v-for="(item, pindex) in data" :key="pindex">
        ...
        <Col span="2" class="left">
          <span class="item" :class="{on: item.checked}" @click="handleResultClick(item, pindex)">{{item.name}}</span>
        </Col>
        <Col span="22" class="right" :style="{'border-bottom': '1px solid #f0f0f0'}">
          <Row v-if="child.children" v-for="(child, cindex) in item.children" :key="cindex" style="border-bottom: 1px solid #f0f0f0">
            <!-- 二级数据 -->
            <Col span="2">
              <span class="item" :class="{on: child.checked}" @click="handleResultClick(child, cindex)">{{child.name}} <Icon v-if="child.children" type="chevron-right"></Icon></span>
            </Col>
            <Col span="22">
              <!-- 三级数据 -->
