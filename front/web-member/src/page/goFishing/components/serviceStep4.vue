    ...
    <div class="add-service">
        <Form ref="data" :model="data" :label-width="120"   label-position="left" :rules="ruleInline">
            ...
            <FormItem label="注意事项" prop="mattres_need_attention">
                <Input type="textarea" v-model="data.mattres_need_attention" :autosize="{minRows: 3,maxRows: 5}" :maxlength="200"></Input>
            </FormItem>
            <FormItem label="承诺内容" prop="promise_content">
                <Input type="textarea" v-model="data.promise_content" :autosize="{minRows: 3,maxRows: 5}" :maxlength="200"></Input>
            </FormItem>
            </Form>
        <div class="tc pt20">
                ...
                this.$api.post('/member/fishing/findFishingService',{id: this.data.id,pageNum:1}).then(response => {
                    if(response.code == 200){
                        if(response.data.list[0]){
                            ...
                            this.data.mattres_need_attention = data.mattres_need_attention ? data.mattres_need_attention : ''
                            this.data.promise_content = data.mattres_need_attention ? data.mattres_need_attention : ''
                    ...
                    if (valid) {
                        this.$api.post('/member/fishing/updateFishingService',this.data).then(response=>{
                            if(response.code == 200){
                                ...
                                this.$router.push(`/addService/step5?id=${this.data.id}`)
                            }
                        })  
                    } else {
