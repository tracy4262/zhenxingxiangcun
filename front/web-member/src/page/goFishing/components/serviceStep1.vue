...
<div class="add-service">
    <Form ref="data" :model="data" :label-width="120" label-position="left" :rules="ruleInline" >
        <FormItem label="通用服务名称" prop="currency_service_name">
            ...
            <!-- <vuiCommodity ref="vuiCommodity" @on-save="onSaveFocus" type="2" :num="1" :values="data.currency_service_name"></vuiCommodity> -->
        </FormItem>
        <FormItem label="行业分类">
    ...
    export default {
        ...
        data() {
            return{
                ...
                ruleInline:{
                    currency_service_name:[{required: true,message:'请输入通用服务名称', trigger: 'change'}],
        ...
        methods:{
            ...
            onSaveFocus (e) {
                if (e.length) {
                    this.data.currency_service_name = e[0].label
                } else {
                    this.data.currency_service_name = ''
                }
            },
            //初始化获取数据
            handleInit(){
                    ...
                    if (valid) {
                        ...
                        if(this.id){
                            ...
                            this.$api.post('/member/fishing/insertFishingService',this.data).then(response=>{
                                if(response.code == 200){
                                    this.$Message.success('保存成功')
                                    this.$router.push(`/addService/step2?id=${response.data}`)
                                }
                            })
                        }
                    } else {
