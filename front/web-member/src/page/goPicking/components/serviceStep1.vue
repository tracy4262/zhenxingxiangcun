    ...
    export default {
        ...
        data() {
            return{
                ...
                serviceNames: [
                    {label: '垂钓', value: '垂钓', type: '娱乐服务'},
                    {label: '采摘', value: '采摘', type: '娱乐服务'},
                    {label: '民宿', value: '民宿', type: '住宿服务'},
                    {label: '农家乐', value: '农家乐', type: '餐饮服务'},
                    {label: '景区', value: '景区', type: '娱乐服务'},
                ]
            }
        },
        created(){
                    ...
                    if (valid) {
                        ...
                        if(this.id){
                            ...
                            this.$api.post('/member/fishing/insertFishingService',this.data).then(response=>{
                                if(response.code == 200){
                                    this.$Message.success('保存成功')
                                    this.$router.push(`/pickingAddService/step2?id=${response.data}`)
                                }
                            })
                        }
                    } else {
