			...
			<Form ref="pubstep1" :model="mydynamic" :rules="ruleInline1" label-position="right" :label-width="180" v-show="process === 2">
				...
				<FormItem prop="standardFile" class="legal_person" label="上传标准文件">
					<Upload ref="upload" :show-upload-list="false"
							name="file"
							:format="['jpg', 'png', 'doc', 'docx', 'pdf', 'xlsx']"
							:max-size="51200"
						...
						<div class="upload-add-div">
							...
							<p class="mb5">点击添加文件</p>
							<p class="t-grey">支持jpg/png/doc/pdf</p>
							<p class="t-grey">/docx格式,不超过50M</p>
						</div>
					</Upload>
					<div class="upload-add-div photo-selector" @click="handlePhotoSelectorModal('standardFile')" style="width: 150px;">
						<Icon type="plus-circled" color="#00c587" :size="32"></Icon>
						<p class="mb5">从文件管理导入</p>
						<p class="t-grey">支持jpg/png格式</p>
						<p class="t-grey">不超过2M</p>
					</div>
					<div class="new-upload-list" v-for="(item,index) in mydynamic.standardFile" :key="index">
						<template v-if="item.status === 'finished'">
							<img style="width:140px; height: 140px;" v-if="item.response.data.type === 'pic'" :src="item.response.data.src">
							<img style="width:140px; height: 140px;" v-else src="../../../../static/datas/img/myStyle/OTH.png">
...
<script>
	...
	export default {
		...
		created() {
			// 获取相册
			this.getAlbum('standardFile', this.l2)
			//获得名字
		...
		methods: {
			...
			publishSubmit (name) {
				...
				let data = {
					...
					standardFile: this.mydynamic.standardFile.length === 0 ? '' : this.mydynamic.standardFile[0].response.data.src,
			...
			handleBeforeUpload () {
                ...
                this.mydynamic.standardFile.length + this.$refs.upload.fileList.length < 1
                if (!check) {
                    this.$Notice.warning({
                        title: '最多只能上传 1 份文件。'
                    })
			...
			handleSuccess(response, file, fileList) {
				...
				if (response.code ==='0') {
                    ...
                    let b = src.length
					let format = src.substring(a, b)
					if (format === '.jpg' || format === '.jpeg' || format === '.png') {
                        file.response.data.type = 'pic'
                    } else if (format === '.pdf') {
                        file.response.data.type = 'file'
                    }
                    this.mydynamic.standardFile.push(file)
				} else {
			...
			handleRemove (file) {
                // 先从服务器上把图片删除
                this.mydynamic.standardFile.splice(0, 1)
			...
			handleFormatError(file) {
                this.$Notice.warning({
                    title: "文件格式有误",
                    desc: "照片格式不正确，请选择jpg、png、doc、docx、pdf格式。",
                    duration: 6
                })
            },
            ...
            handleGetPhotoResult2 (result) {
                // 用于限制最大可上传照片的张数
                if (this.mydynamic.standardFile.length + result.length > 1) {
                    this.$Notice.warning({
                        title: '最多只能上传 1 份文件。'
                    })
                } else {
                    result.forEach(element => {
                        this.mydynamic.standardFile.push({
                            response: {
                                data: {
									src: element,
									type: 'pic'
