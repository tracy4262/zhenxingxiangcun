          ...
          <Col span="6" v-for="(item, index) in data" class="pb20" :key="item">
            <Card >
              <div @click="handleDeatil(item)" style="height: 281px;">
                ...
                <div class="pl10 pr10">
                    <p class="pt10 ell" :title="item.productName">{{item.productName}}</p>
                    ...
                    <p class="pt5">
                        <span v-if="item.durationPrice && item.productPrice">价格：<span class="t-orange">{{item.durationPrice ? item.durationPrice : item.productPrice}}</span>元/{{item.unit}}</span>
                    </p>
                    <p class="pb15 pt5 ell" :title="item.season" v-if="type == '1'">
                        采摘季节：{{item.season}}
                    </p>
                </div>
              </div>
