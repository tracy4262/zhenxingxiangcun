    ...
    <Row type="flex" justify="center" class="mt20">
      ...
      <div style="width:1200px">
        <Breadcrumb>
          ...
          <BreadcrumbItem
            :to="parentAddress"
            v-if="parentCode != '' && parentCode != undefined"
            @click.native="reload"
          >{{parentName}}</BreadcrumbItem>
          <BreadcrumbItem to="/goods/showGoods">{{productName}}</BreadcrumbItem>
        </Breadcrumb>
        <filter-btn ref="btn" @on-search="handleOnchange" :btnData2="btnData"></filter-btn>
...
export default {
  ...
  computed: {
    parentAddress() {
      return (
        "/goods/showGoods?name=" + this.parentName + "&code=" + this.parentCode
      );
    }
  },
  methods: {
    ...
    searchBtn() {
      this.$api
        .post("/shop/pushShopCommodity/findProduct", {
