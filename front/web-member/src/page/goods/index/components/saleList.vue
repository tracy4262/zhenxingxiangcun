    ...
    <Row type="flex" justify="center" class="mt20">
      ...
      <div style="width:1200px">
        <Breadcrumb v-show="isShow">
          ...
          <BreadcrumbItem to="/goods/showGoods">定价商品</BreadcrumbItem>
        </Breadcrumb>
        ...
        <ul class="goods-list" v-if="list!=null">
          ...
          >
            ...
            <Row type="flex" justify="space-between" class="mt10 ml15">
              <Col>
                <span style="font-size:16px;color:red">￥{{item.currentPrice}}</span>
          ...
          <li v-for="item in list" v-if="isShow" @click="handleDetail(item)">
            ...
            >
              距离结束还剩：
              <vui-clocker :time="item.discountPeriodStr[1]" format="%D天 %H小时 %M分 %S秒"/>
            </div>
            <img :src="item.notarizationCertificate[0]" class="imgSet">
            <Row type="flex" justify="space-between" class="mt10 ml15">
              <Col>
...
export default {
  ...
  methods: {
    ...
    searchBtn() {
      this.$api
        .post("/shop/pushShopCommodity/findPricing", {
