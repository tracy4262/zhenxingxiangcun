...
export default {
    ...
    methods: {
        // 营业网点数据初始化
        handleInit () {
            // 必须在调用此组件的组件中调用此方法，否则会拿不到 marketingId 导致请求出错
            this.$api.post('/shop/commodityMarketing/networkStationList', {
                account: this.$user.loginAccount,
                marketingId: this.item.id === undefined ? '' : this.item.id,
                type: this.type // 1对应发货信息  2对应售后信息
            }).then(response => {
