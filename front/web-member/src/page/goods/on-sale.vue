<template>
    <div>
        ...
        <section style="background: #F9F9F9">
            <goods-head title="正在出售">
                <BreadcrumbItem>正在出售</BreadcrumbItem>
            </goods-head>
            <div class="layouts">
                <section class="bg-white mt20 mb20 pd30">
                    <Form ref="info" :model="info" label-position="left" :label-width="90" :rules="ruleInline">
                        ...
                        <Row>
                            <Col span="12">
                                <FormItem label="批量操作">
                                    <Button type="default" size="small" @click="down">下架</Button>
                                </FormItem>
                            </Col>
                        </Row>
                    </Form>
        ...
        <Modal v-model="modal" width="560" title="修改价格" :mask-closable="false">
            <Form ref="priceInfo" :model="priceInfo" label-position="right" :label-width="150" :rules="ruleInline2" class="mt20">
                <Form-item prop="price" label="时价">
                    <Input v-model="priceInfo.price" style="width: 300px;">
                ...
                <Form-item prop="discountPrice" label="折扣价">
                 ...
                 <Form-item prop="time" label="折扣时限">
                    <DatePicker placeholder="折扣时限" style="width:300px;" type="datetime" :options="options3" :editable="false" v-model="priceInfo.time" ></DatePicker>
                </Form-item>
            </Form>
            <div slot="footer">
...
export default {
                    ...
                    render: (h, params) => {
                        return h('div', [
                            h('Button', {
                                ...
                                on: {
                                    click: () => {
                                        this.downOne(params.row.commodityId)
                                    }
                                }
                            }, '下架')
                        ]);
    ...
    created () {
        ...
        this.getData({
            ...
            storageStatus: '3', //货架状态   0:未上架,1:我下架的,2:售完下架,3:上架到正在出售页面',4:货架查全部
            // approveStatus: '4', //审核状态   0 审核中，1已通过，2未通过，4空为全部
            commodityName: '', //商品名称
            productTypeId: '', //商品分类
            startPrice: '', //最小价格
            endPrice: '', //最大价格
            onSaleTime: '0', //按删除时间排序，0：降序，1：升序
    ...
    methods: {
        ...
        query () {
            ...
            let data = {
                ...
                onSaleTime: this.info.inventory !== '' ? '' : this.info.onSaleTime === '' ? '0' : this.info.onSaleTime === 'asc' ? '1' : this.info.onSaleTime === 'desc' ? '0' : '0', //按创建时间排序，0：降序，1：升序
        ...
        editPrice (row) {
            ...
            this.priceInfo.price = row.productPrice.split('：')[1]
            this.priceInfo.discountPrice = row.discountPrice.split('：')[1]
            this.priceInfo.time = row.discountTime.split('：')[1]
        },
        ok () {
            console.log('this.priceInfo', this.priceInfo)
            this.$api.post('/portal/shopStorage/updateStoragePrice', {
                account: this.$user.loginAccount,
                commodityId: this.priceInfo.commodityId,
                timePrice: this.priceInfo.price, 
                discountPrice: this.priceInfo.discountPrice,
                discountDate: this.moment(this.priceInfo.time).format('YYYY/MM/DD HH:mm:ss')
            }).then(response => {
                console.log('response', response)
