...
<template>
    <div class="mt20 ml10 mr10 proxy-card-shadow">
        ...
        <Row class="proxy-button-bar tc" type="flex" align="middle">
            <Col span="12">
                <a class="proxy-button disabled" v-if="item.account === $user.loginAccount">不能邀请自己</a>
                <a class="proxy-button" v-else-if="item.status === '邀请'" @click="invite">{{ item.status }}</a>
    ...
    methods: {
        invite () {
            this.$Modal.confirm({
                ...
                cancelText:'取消',
                onOk: () => {
                    this.$api.post('/member-reversion/employ/invite', {
                        sendAccount: this.$user.loginAccount, // 发送邀请方账号
                        beInvitedAccount: this.item.account // 受邀方账号
                    }).then(response => {
        ...
        detail () {
            window.open('/portals/index?uid='+this.item.account)
        }
    }
}
</script>
