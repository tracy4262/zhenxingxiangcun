  ...
  <div style="background:#f9f9f9;" class="mb40">
   <!-- 第一产业 -->
        <div class="pd20" >
          ...
          <Form :label-width="75" label-position="left"  v-for="(item, index) in data" :ref="`data${index}`" :model="item" :rules="dataLine" :key="index">
            <Row :gutter="16">
              <Col span="12">
    ...
    methods: {
      ...
      handleDel (item, index) {
        this.$Modal.confirm({
            ...
            onOk:()=>{
              if (item.id) {
                  this.$api.post('/member-reversion/productionBase/ecoSocial/deleteServiceProduct', {id: item.id}).then(response => {
      ...
      handleSave () {
        ...
        if (flag) {
          ...
          this.$api.post('/member-reversion/productionBase/ecoSocial/saveServiceProduct', list).then(response => {
              if (response.code === 200) {
