  ...
  <div>
    ...
    <div style="background:#f9f9f9;" class="mb40">
      <div class="pd20" >
          ...
          <Form :label-width="60" label-position="left" v-for="(item, index) in data" :ref="`data${index}`" :model="item" :rules="dataLine"  :key="index">
            <Row  :gutter="16" >
              <Col span="8">
                <FormItem label="物种名称">
    ...
    methods: {
      ...
      handleDel (item, index) {
        this.$Modal.confirm({
            ...
            onOk:()=>{
              if (item.id) {
                  this.$api.post('/member-reversion/productionBase/ecoSocial/deleteBreed', {id: item.id}).then(response => {
      ...
      handleSave () {
        ...
        if (flag) {
          ...
          this.$api.post('/member-reversion/productionBase/ecoSocial/saveBreed', list).then(response => {
              if (response.code === 200) {
