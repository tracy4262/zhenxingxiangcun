...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 初始化加载数据
            init () {
                this.$api.post('/member-reversion/productionBase/landInfo/findWaterQuality',{
                    account: this.$user.loginAccount,
                    dictId: this.id,
                    baseId: this.baseId
                }).then(response => {
            ...
            handleSave (item, index) {
                this.$refs[`data${index}`][0].validate(v => {
                    if (!v) {
                        ...
                        this.loading = true
                        this.$api.post('/member-reversion/productionBase/landInfo/updateWaterQuality', item).then(response => {
                            if (response.code === 200) {
