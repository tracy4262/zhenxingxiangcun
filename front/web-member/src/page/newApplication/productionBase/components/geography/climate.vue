...
export default {
  ...
  methods: {
    ...
    //初始化取数据
    handleInit () {
      this.$api.post('/member-reversion/productionBase/physicalGeography/findClimateInfo', {
    ...
    handleSave () {
      ...
      this.loading = true
      this.$api.post('/member-reversion/productionBase/physicalGeography/saveClimateInfo', list).then(response => {
        if (response.code === 200) {
