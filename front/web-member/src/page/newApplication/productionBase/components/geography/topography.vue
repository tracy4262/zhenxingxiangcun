...
export default {
  ...
  methods: {
    ...
    //初始化取数据
    handleInit () {
      this.$api.post('/member-reversion/productionBase/physicalGeography/findTopographicFeature', {
    ...
    handleSave () {
      this.$refs['data'].validate(v => {
          if (v) {
            ...
            this.loading = true
            this.$api.post('/member-reversion/productionBase/physicalGeography/saveTopographicFeature', list).then(response => {
              if (response.code === 200) {
