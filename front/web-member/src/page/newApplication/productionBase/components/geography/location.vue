<template>
  <div class="pd20">
    ...
    <Form :label-width="100" label-position="left" ref="data" class="ml20 mt20">
      <Title title="会员所在地" edit :id="id"></Title>
      <div class="pd20">
          <FormItem label="所在位置">
            <Row>
              <Col span="20">
      ...
      <Title title="会员所在地经纬度" edit :id="id"></Title>
      <div class="pd20  pb40">
          <Row :gutter="38">
            <Col span="8">
              <FormItem label="东经">
      ...
      <Title title="会员四邻" edit :id="id"></Title>
      <div class="pd20  pb40">
        <Row :gutter="16" class="four-neighbors">
          <Col span="24" class="pb20" v-for="(item, index) in memberNeighbor.memberNeighbor" :key="index">
            <span>
      ...
      <Title title="查看实况地址" edit :id="id"></Title>
      <div class="pd20 pb40">
        <Row  :gutter="24" class="pb20" v-for="(item, index) in memberLiveAddress.memberLiveAddress" :key="index">
          <Col span="1">
            {{index+1}}、
          </Col>
          <Col span="8">
...
export default {
  ...
  data () {
    return {
      titles: ['会员所在地', '会员所在地经纬度', '会员四邻', '查看实况地址'],
  ...
  methods: {
    ...
    //初始化获取收藏夹数据
    handleInit(){
        this.$api.post('/member-reversion/productionBase/physicalGeography/findGeographicalPosition', {
    ...
    onSave () {
      ...
      this.$api.post('/member-reversion/productionBase/physicalGeography/saveGeographicalPosition', list).then(response => {
        if (response.code === 200) {
    ...
    fourNeighborsDel (item, index) {
      this.$Modal.confirm({
          ...
          onOk:()=>{
            if (item.id) {
              this.$api.post('/member-reversion/productionBase/physicalGeography/deleteMemberNeighbor',{id: item.id}).then(response => {
                if (response.code === 200) {
    ...
    handleDelLiveAddress (item, index) {
      this.$Modal.confirm({
          ...
          onOk:()=>{
            if (item.id) {
              this.$api.post('/member-reversion/productionBase/physicalGeography/deleteMemberLiveAddress',{id: item.id}).then(response => {
                if (response.code === 200) {
