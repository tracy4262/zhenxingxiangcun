        ...
        <Row :gutter="38">
          <Col span="8">
            <FormItem label="会员全称" prop="member_name">
              ...
              <!-- <Input v-model="item.member_name" :disabled="!item.isEdit" ></Input> -->
            </FormItem>
          </Col>
          <Col span="8">
            <FormItem label="名称简写" prop="member_abbreviation">
              ...
              <!-- <Input v-model="item.member_abbreviation" :disabled="!item.isEdit" ></Input> -->
            </FormItem>
          </Col>
          <Col span="8">
    ...
    methods: {
      ...
      handleInit (e) {
        ...
        this.$api.post('/member-reversion/user/realCertification/findMemberIdentity', {
          ...
          if (response.code === 200) {
            ...
            this.$nextTick(e => {
              for(var i = 0 ;i < data.length ; i++){
                  var list = this.data[i].card_image
      ...
      handleDel (item, index) {
        this.$Modal.confirm({
            ...
            // content: '是否确认删除？',
            onOk:()=>{
              if (item.member_identity_info_id) {
                this.$api.post('/member-reversion/user/realCertification/deleteMemberIdentity',{member_identity_info_id: item.member_identity_info_id}).then(response => {
                  if (response.code === 200) {
      ...
      handleSave (item, index) {
        if (!this.isloading) {
          this.isloading = true
          let url = ''
          if (item.member_identity_info_id) { // 更新
            url = '/member-reversion/user/realCertification/updateMemberIdentity'
          } else { // 保存
            url = '/member-reversion/user/realCertification/saveMemberIdentity'
          }
