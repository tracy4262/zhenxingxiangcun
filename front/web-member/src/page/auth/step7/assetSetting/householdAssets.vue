          ...
          <Row :gutter="16">
              ...
              <Col span="8">
                  <Form-item prop="unit" label="单位">
                    <vuiUnit :value="item.unit" @on-get-data="onGetUnit($event, index)" :disabled="!item.edit"></vuiUnit>
                  </Form-item>
              </Col>
            </Row>
            <Row :gutter="16">
  ...
  methods: {
    ...
    handleDel (item, index) {
       this.$Modal.confirm({
            ...
            onOk:()=>{
                if (item.id) {
                    this.$api.post('/member-reversion/assetSeting/deleteHouseholdAssetsInfo', {id: item.id}).then(response => {
                        if (response.code === 200) {
        ...
        this.$refs[`data${index}`][0].validate(v => {
          if (v) {
            let list = {
                ...
                yearId: this.yearId,
                parentId: this.id,
                householdAssetsInfoName: this.title,
                householdAssetsInfo: item,
                templateId: this.templateId
            }
            this.$api.post('/member-reversion/assetSeting/saveHouseholdAssetsInfo', list).then(response => {
