<template>  
  <div  class="pd20">
     <Title :title="title" edit :id="id" :yearId="yearId" :templateId="templateId" >
          ...
          <Row :gutter="16">
              <Col span="8">
                  <Form-item prop="networkName" label="网点名称">
                      <Input v-model="item.networkName" :maxlength="20" :disabled="!item.edit" @on-change="changePreview"></Input>       
                  </Form-item>
              </Col>
              <Col span="8">
                  <Form-item prop="networkType" label="网点类型">
                    <CheckboxGroup v-model="item.networkType"  @on-change="changePreview">
                        <Checkbox label="销售门店" :disabled="!item.edit"></Checkbox>
                        <Checkbox label="售后网点" :disabled="!item.edit"></Checkbox>
                    </CheckboxGroup>
                  </Form-item>
              </Col>
          </Row>
           <Row :gutter="16">
              <Col span="16">
                  <Form-item prop="location" label="网点所在地" @click.native="activeMode = index">
                    <Input v-model="item.location" readonly :disabled="!item.edit"></Input>
                    <!-- <vui-cascander :values="item.location" @handle-get-result="handleGetData" :disabled="!item.edit"></vui-cascander> -->
                  </Form-item>
              </Col>
              <Col span="24">
                  <Form-item  label="详细地址">
                      <Row>
                        <Col span="10" class="pr10">
                          <Input v-model="item.address" :maxlength="50" :disabled="!item.edit" placeholder="请输入所在街道/湾组/楼栋" @on-change="changePerfectAddress(item, index)"></Input>
                        </Col>
                        <Col span="5" class="pl10">
                          <Input v-model="item.houseNumber" :maxlength="20" :disabled="!item.edit" placeholder="请输入详细门牌号"  @on-change="changePerfectAddress(item, index)"></Input>
                        </Col>
                      </Row>
                  </Form-item>
              </Col>
          ...
          <Row :gutter="16">
              <Col span="8">
                  <Form-item prop="contact" label="联系人">
                    <Input v-model="item.contact" :maxlength="20" @on-change="changePreview" :disabled="!item.edit"></Input>
                  </Form-item>
              </Col>
              <Col span="8">
                  <Form-item prop="officePhone" label="办公电话">
                    <Input v-model="item.officePhone" :maxlength="20" @on-change="changePreview" :disabled="!item.edit"></Input>
                  </Form-item>
              </Col>
              <Col span="8">
                  <Form-item prop="phone" label="手机号码">
                    <Input v-model="item.phone" :maxlength="20" @on-change="changePreview" :disabled="!item.edit"></Input>
                  </Form-item>
              </Col>
          </Row>
          <Row :gutter="16">
            <Col span="8">
              <FormItem label="东经">
                <Input v-model="item.longitude" readonly  @on-change="changePreview" :disabled="!item.edit"></Input>
              </FormItem>
            </Col>
            <Col span="8">
              <FormItem label="北纬">
                <Input v-model="item.latitude" readonly  @on-change="changePreview" :disabled="!item.edit"></Input>
              </FormItem>
            </Col>
            <!-- <Col span="8" style="text-decoration: underline; color: #6C6C6C; cursor: pointer;" class="pt5">
              ...
              <span v-else>定位获取</span>
            </Col> -->
          </Row>
          <div class="pb30 pt10" v-if="item.latitude">
            <a target="_blank" :href="`http://api.map.baidu.com/marker?location=${item.latitude},${item.longitude}&title=我的位置&content=${item.location}&output=html`">
...
export default {
  ...
  methods: {
    // 点击更新数据
    handleUpdate () {
      this.$api.post('/member-reversion/businessOutlets/updateGisYywd',{
