...
<script>
    ...
    export default {
        ...
        methods: {
            // 点击更新数据
            handleUpdate () {
                this.$api.post('/member-reversion/cultureSight/updateGisFJMS',{
            ...
            // 初始化加载数据 type=0时默认 type=1时重新生成文字预览
            init (type = 0) {
                this.$api.post('/member-reversion/cultureSight/findFamousScenery', {
                    account: this.$user.loginAccount,
                    yearId: this.yearId,
                    dictId: this.modeId,
                    templateId: this.templateId
                ...
                this.$Modal.confirm({
                    ...
                    onOk:()=>{
                        if (item.id !== undefined && item.id !== '') {
                            this.$api.post('/member-reversion/cultureSight/deleteScenery', {
                                id: item.id
                            }).then(response => {
                                if (response.code === 200) {
            ...
            save (item, index) {
                if (!this.saveisloading) {
                    ...
                    this.$api.post('/member-reversion/cultureSight/saveScenery', data).then(response => {
            ...
            change () {
                this.preview = ''
                this.preview += `全村风景名胜区有 ${this.data.length} 处，其中：`
                this.data.forEach(element => {
                    if (element.name && element.name !== '') {
                        this.preview += `${element.name}风景区，`
                    }
