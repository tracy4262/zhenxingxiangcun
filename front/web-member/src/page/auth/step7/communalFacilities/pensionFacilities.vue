...
<script>
    ...
    export default {
        ...
        methods: {
            // 点击更新数据
            handleUpdate () {
                this.$api.post('/member-reversion/communalFacilities/updateYlfl',{
            ...
            // 初始化加载数据
            init (type = 0) {
                this.$api.post('/member-reversion/communalFacilities/findOldAgeWelfareFacilitiesInfo', {
                    account: this.$user.loginAccount,
                    yearId: this.yearId,
                    ...
                    if (response.code === 200) {
                        ...
                        // if (response.data.oldAgeWelfareFacilitiesInfoName && response.data.oldAgeWelfareFacilitiesInfoName !== '') {
                        //     this.title = response.data.oldAgeWelfareFacilitiesInfoName
                        // }
                        ...
                        this.data = []
                        if (response.data.oldAgeWelfareFacilitiesInfo.length !== 0) {
                            response.data.oldAgeWelfareFacilitiesInfo.forEach((element, index) => {
                                let no
                                if (index === 0) {
                                    no = '01'
                                } else {
                ...
                this.$Modal.confirm({
                    ...
                    onOk:()=>{
                        if (item.id !== undefined && item.id !== '') {
                            this.$api.post('/member-reversion/communalFacilities/deleteOldAgeWelfareFacilitiesInfo', {
                                id: item.id
                            }).then(response => {
            ...
            save (item, index) {
                if (!this.saveisloading) {
                    ...
                    let data = {
                        ...
                        yearId: this.yearId,
                        parentId: this.modeId,
                        oldAgeWelfareFacilitiesInfoName: this.title,
                        templateId: this.templateId,
                        oldAgeWelfareFacilitiesInfo: {
                    ...
                    this.$api.post('/member-reversion/communalFacilities/saveOldAgeWelfareFacilitiesInfo', data).then(response => {
