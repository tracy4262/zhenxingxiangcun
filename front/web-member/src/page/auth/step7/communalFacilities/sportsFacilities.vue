...
<script>
    ...
    export default {
        ...
        methods: {
            // 点击更新数据
            handleUpdate () {
                this.$api.post('/member-reversion/communalFacilities/updateTyss',{
            ...
            // 初始化加载数据
            init (type = 0) {
                this.$api.post('/member-reversion/communalFacilities/findSportsFacilitiesInfo', {
                    account: this.$user.loginAccount,
                    yearId: this.yearId,
                    ...
                    if (response.code === 200) {
                        ...
                        // if (response.data.sportsFacilitiesInfoName && response.data.sportsFacilitiesInfoName !== '') {
                        //     this.title = response.data.sportsFacilitiesInfoName
                        // }
                        ...
                        this.data = []
                        if (response.data.sportsFacilitiesInfo.length !== 0) {
                            response.data.sportsFacilitiesInfo.forEach((element, index) => {
                                let no
                                if (index === 0) {
                                    no = '01'
                                } else {
                ...
                this.$Modal.confirm({
                    ...
                    onOk:()=>{
                        if (item.id !== undefined && item.id !== '') {
                            this.$api.post('/member-reversion/communalFacilities/deleteSportsFacilitiesInfo', {
                                id: item.id
                            }).then(response => {
            ...
            save (item, index) {
                if (!this.saveisloading) {
                    ...
                    let data = {
                        ...
                        yearId: this.yearId,
                        parentId: this.modeId,
                        sportsFacilitiesInfoName: this.title,
                        templateId: this.templateId,
                        sportsFacilitiesInfo: {
                    ...
                    this.$api.post('/member-reversion/communalFacilities/saveSportsFacilitiesInfo', data).then(response => {
