<template>
  <div class="pd20">
    <Title :title="title" edit :id="modeId" :yearId="yearId" :templateId="templateId" @left-refresh="leftRefresh" >
        <!-- <Button type="primary" size="small" @click="handleUpdate">更新数据</!--> -->
    </Title>
            ...
            <Row>
                <Col span="24">
                  <Form-item label="设施地址">
                      <Input v-model="item.address" :maxlength="100" :disabled="!item.isAdd" @on-change="change" />
                  </Form-item>
                </Col>
            </Row>
            <Row :gutter="32">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 初始化加载数据
            init (type = 0) {
                this.$api.post('/member-reversion/communalFacilities/findIotDeviceInfo', {
                    account: this.$user.loginAccount,
                    yearId: this.yearId,
                    ...
                    if (response.code === 200) {
                        ...
                        if (response.data.iotDeviceInfo.length !== 0) {
                            this.data = []
                            response.data.iotDeviceInfo.forEach((element, index) => {
                                let no
                                if (index === 0) {
                                    no = '01'
                                } else {
                ...
                this.$Modal.confirm({
                    ...
                    onOk:()=>{
                        if (item.id !== undefined && item.id !== '') {
                            this.$api.post('/member-reversion/communalFacilities/deleteIotDeviceInfo', {
                                id: item.id
                            }).then(response => {
            ...
            save (item, index) {
                if (!this.saveisloading) {
                    ...
                    this.$api.post('/member-reversion/communalFacilities/saveIotDeviceInfo', data).then(response => {
            ...
            change () {
                ...
                this.data.forEach(element => {
                    if (element.name && element.name !== '') {
                        this.preview += `物联设施${element.name}，`
                    }
                    if (element.capacity && element.capacity !== '') {
                        this.preview += `设施规格是${element.capacity}，`
                    }
                    if (element.investment && element.investment !== '') {
                        this.preview += `投资金额是${element.investment}万元；`
                    }
                })
