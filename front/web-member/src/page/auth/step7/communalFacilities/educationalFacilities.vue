...
<script>
    ...
    export default {
        ...
        methods: {
            // 点击更新数据
            handleUpdate () {
                this.$api.post('/member-reversion/communalFacilities/updateJyss',{
            ...
            // 初始化加载数据
            init (type = 0) {
                this.$api.post('/member-reversion/communalFacilities/findEducationFacilitiesInfo', {
                    account: this.$user.loginAccount,
                    yearId: this.yearId,
                    ...
                    if (response.code === 200) {
                        ...
                        // if (response.data.educationFacilitiesInfoName && response.data.educationFacilitiesInfoName !== '') {
                        //     this.title = response.data.educationFacilitiesInfoName
                        // }
                        ...
                        this.data = []
                        if (response.data.educationFacilitiesInfo.length !== 0) {
                            response.data.educationFacilitiesInfo.forEach((element, index) => {
                                let no
                                if (index === 0) {
                                    no = '01'
                                } else {
                ...
                this.$Modal.confirm({
                    ...
                    onOk:()=>{
                        if (item.id !== undefined && item.id !== '') {
                            this.$api.post('/member-reversion/communalFacilities/deleteEducationFacilitiesInfo', {
                                id: item.id
                            }).then(response => {
            ...
            save (item, index) {
                if (!this.saveisloading) {
                    ...
                    let data = {
                        ...
                        yearId: this.yearId,
                        parentId: this.modeId,
                        educationFacilitiesInfoName: this.title,
                        templateId: this.templateId,
                        educationFacilitiesInfo: {
                    ...
                    this.$api.post('/member-reversion/communalFacilities/saveEducationFacilitiesInfo', data).then(response => {
