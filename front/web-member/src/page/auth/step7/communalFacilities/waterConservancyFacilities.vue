                ...
                <Col span="8">
                    <Form-item label="通用名称">
                        <Select v-model="item.commonName" :disabled="!item.isAdd" @on-change="change">
                            ...
                            <Option value="塘堰">塘堰</Option>
                            <Option value="沟渠">沟渠</Option>
...
<script>
    ...
    export default {
        ...
        methods: {
            // 点击更新数据
            handleUpdate () {
                this.$api.post('/member-reversion/communalFacilities/updateGisSlss',{
            ...
            // 初始化加载数据
            init (type = 0) {
                this.$api.post('/member-reversion/communalFacilities/findWaterConservancyFacilitiesInfo', {
                    account: this.$user.loginAccount,
                    yearId: this.yearId,
                    ...
                    if (response.code === 200) {
                        ...
                        // if (response.data.waterConservancyFacilitiesInfoName && response.data.waterConservancyFacilitiesInfoName !== '') {
                        //     this.title = response.data.waterConservancyFacilitiesInfoName
                        // }
                        ...
                        this.data = []
                        if (response.data.waterConservancyFacilitiesInfo.length !== 0) {
                            response.data.waterConservancyFacilitiesInfo.forEach((element, index) => {
                                let no
                                if (index === 0) {
                                    no = '01'
                                } else {
                ...
                this.$Modal.confirm({
                    ...
                    onOk:()=>{
                        if (item.id !== undefined && item.id !== '') {
                            this.$api.post('/member-reversion/communalFacilities/deleteWaterConservancyFacilitiesInfo', {
                                id: item.id
                            }).then(response => {
            ...
            save (item, index) {
                if (!this.saveisloading) {
                    ...
                    let data = {
                        ...
                        yearId: this.yearId,
                        parentId: this.modeId,
                        waterConservancyFacilitiesInfoName: this.title,
                        templateId: this.templateId,
                        waterConservancyFacilitiesInfo: {
                    ...
                    this.$api.post('/member-reversion/communalFacilities/saveWaterConservancyFacilitiesInfo', data).then(response => {
