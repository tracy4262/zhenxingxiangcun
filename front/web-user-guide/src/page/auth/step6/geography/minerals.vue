  ...
  <div class="pd20">
    ...
    <Form :label-width="100" label-position="left" class="pd20 mt40">
      ...
      <FormItem :label="item.minerals_class" v-for="(item, index) in data">
        <!-- 能源矿产 -->
        <Select v-model="item.minerals_name" multiple style="width:300px"  :disabled="true"  @on-open-change="changePreview" v-if="index == 0">
            <Option v-for="list in energy" :value="list.minerals_name" :key="list.id">{{ list.minerals_name }}</Option>
        </Select>
        <!-- 金属矿产 -->
         <Select v-model="item.minerals_name" multiple style="width:300px"  :disabled="true"  @on-open-change="changePreview" v-if="index == 1">
            <Option v-for="list in metal" :value="list.minerals_name" :key="list.id">{{ list.minerals_name }}</Option>
        </Select>
        <!-- 非金属矿产 -->
         <Select v-model="item.minerals_name" multiple style="width:300px"  :disabled="true"  @on-open-change="changePreview" v-if="index == 2">
            <Option v-for="list in nonmetal" :value="list.minerals_name" :key="list.id">{{ list.minerals_name }}</Option>
        </Select>
        <!-- 水气矿产 -->
         <Select v-model="item.minerals_name" multiple style="width:300px"  :disabled="true"  @on-open-change="changePreview" v-if="index == 3">
    ...
    handleInit () {
      this.$api.post('/member-reversion/physicalGeography/findMineralsInfo',{
       templateId: this.$template.id, user_id: this.$user.loginAccount, year_id: this.yearId, parent_id: this.id
        ...
        if (response.code === 200) {
          this.title = response.data.minerals_name
          ...
          if (!response.data.textPreview.text_preview) {
            response.data.textPreview.text_preview = `所在地已发现矿产（）种，其中：（）有（）。`
          }
        }
      })
    },
