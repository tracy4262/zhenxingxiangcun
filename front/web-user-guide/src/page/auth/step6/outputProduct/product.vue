          ...
          <Row :gutter="16">
              <Col span="8">
                  <Form-item prop="productName" label="产品名称">
                      <Input v-model="item.productName" :maxlength="20" :disabled="!item.edit" ></Input>
                  </Form-item>
              </Col>
              <Col span="8">
                  <Form-item prop="units" label="产量单位">
                    <vuiUnit :value="item.units" @on-get-data="onGetUnit($event, index)" :disabled="!item.edit"></vuiUnit>
                      <!-- <Select v-model="item.units"  filterable style="width: 100%" class="units"  placeholder="单位" :disabled="!item.edit">
              ...
              <Col span="8">
                  <Form-item prop="product" label="产品产量">
                      <Input v-model="item.product" :maxlength="20" :disabled="!item.edit"></Input>
                  </Form-item>
              </Col>
              <Col span="8">
                  <Form-item prop="conversion" label="折算产量">
                      <Input v-model="item.conversion" :maxlength="20" :disabled="!item.edit"></Input>
                  </Form-item>
              </Col>
          </Row>
...
export default {
  ...
  mounted () {
    this.preview = `物种（），生产规模（），通用商品名（），产品名称（），产量单位（），折算产量（）。`
  }
}
</script>
