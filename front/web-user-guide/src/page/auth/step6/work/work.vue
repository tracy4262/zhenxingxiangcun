            ...
            <Form :ref="`formItem${index}`" :model="item" label-position="left" :label-width="100" :rules="formItemInline">
                ...
                <Row :gutter="32">
                    ...
                    <Form-item label="工作时间">
                        <DatePicker v-model="item.time" :editable="false" type="daterange" :options="options" :disabled="!item.isAdd" style="width: 100%;" @on-change="change"></DatePicker>
                    </Form-item>
                    </Col>
                </Row>
...
export default {
  ...
  methods: {
    ...
    init(type = 0) {
      this.$api
        .post('/member-reversion/workExperience/findWorkExperience', {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          if (item.id !== undefined && item.id !== '') {
            this.$api
              .post('/member-reversion/workExperience/deleteWorkExperience', {
                id: item.id
              })
              .then(response => {
                if (response.code === 200) {
    ...
    save(item, index) {
      let arr = [this.moment(item.time[0]).format('YYYY-MM-DD'), this.moment(item.time[1]).format('YYYY-MM-DD')]
      this.$api
        .post('/member-reversion/workExperience/saveWorkExperience', {
          user_id: this.$user.loginAccount,
          yearId: this.yearId,
    ...
    handleSave() {
      this.$api
        .post('/member-reversion/workExperience/saveTextPreview', {
          user_id: this.$user.loginAccount,
          yearId: this.yearId,
    ...
    change() {
      ...
      this.data.forEach(element => {
        ...
        if (element.time && element.time.length > 0) {
          this.preview += `工作时间：${this.moment(element.time[0]).format('YYYY-MM-DD')} - ${this.moment(
            element.time[1]
          ).format('YYYY-MM-DD')}，`
        }
  ...
  mounted() {
    this.preview = `工作单位（），工作职位（），工作时间（），工作详情（）。`
  }
}
</script>
