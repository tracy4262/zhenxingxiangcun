...
<script>
    ...
    export default {
        ...
        methods: {
            // 初始化加载数据
            init (type = 0) {
                this.$api.post('/member-reversion/communalFacilities/findOtherFacilitiesInfo', {
                    ...
                    if (response.code === 200) {
                        ...
                        if (response.data.otherFacilitiesInfoName && response.data.otherFacilitiesInfoName !== '') {
                            this.title = response.data.otherFacilitiesInfoName
                        }
                        ...
                        if (response.data.otherFacilitiesInfo.length !== 0) {
                            this.data = []
                            response.data.otherFacilitiesInfo.forEach((element, index) => {
                                this.data.push({
                                    id: element.id,
                ...
                this.$Modal.confirm({
                    ...
                    onOk:()=>{
                        if (item.id !== undefined && item.id !== '') {
                            this.$api.post('/member-reversion/communalFacilities/deleteOtherFacilitiesInfo', {
                                id: item.id
                            }).then(response => {
                                if (response.code === 200) {
            ...
            save (item, index) {
                let data = {
                    ...
                    yearId: this.yearId,
                    parentId: this.modeId,
                    otherFacilitiesInfoName: this.title,
                    otherFacilitiesInfo: {
                ...
                this.$api.post('/member-reversion/communalFacilities/saveOtherFacilitiesInfo', data).then(response => {
                    if (response.code === 200) {
