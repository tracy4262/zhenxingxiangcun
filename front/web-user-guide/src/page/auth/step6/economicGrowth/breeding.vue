...
export default {
  ...
  methods: {
    ...
    changePreview () {
      let str = ''
      if (this.total) {
        str += `全村种养信息总产值${this.total}万元。其中：农业（农作物种植业）主要种植${this.$refs['agriculture'].preview}产值达到${this.$refs['agriculture'].total}万元；其中：林业产主要种植${this.$refs['forestry'].preview}值达到${this.$refs['forestry'].total}万元；其中：畜牧业主要种植${this.$refs['animal'].preview}产值达到${this.$refs['animal'].total}万元；其中，水产业主要种植${this.$refs['water'].preview}产值达到${this.$refs['water'].total}万元。`
      }
      this.preview = str
    },
    handleNumAdd () {
            ...
            if (response.code == 200) {
              ...
              if (type == 1) {
                ...
                if (response.data.preview) {
                  ...
                  this.preview = `全村种养信息总产值（）万元。其中：农业（农作物种植业）主要种植（）产值达到（）万元；其中：林业产主要种植（）值达到（）万元；其中：畜牧业主要种植（）产值达到（）万元；其中，水产业主要种植（）产值达到（）万元。`
                }
