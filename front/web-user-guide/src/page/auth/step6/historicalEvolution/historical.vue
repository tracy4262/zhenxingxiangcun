        ...
        <div v-for="(item, index) in data">
          <Form :label-width="100" label-position="left" ref="data" :model="item" :rules="dataLine" class="pd20 mt40" style="background:#f9f9f9;">
            ...
            <Row  :gutter="38">
              <Col span="8">
                <FormItem label="时间">
                  <DatePicker type="date" v-model="item.history_time" placeholder="请选择" :disabled="!item.edit" @on-change="handleChange"></DatePicker>
                </FormItem>
              </Col>
              <Col span="16">
...
export default {
  ...
  methods: {
    handleInit () {
      this.$api.post('/member-reversion/historyEvolution/findHistoryEvolution', {templateId: this.$template.id, user_id: this.account, year_id: this.yearId, parent_id: this.id}).then(response => {
        if (response.code === 200) {
          this.handleInits(response.data)
        }
      })
    },
    handleInits (data) {
      ...
      if (!data.textPreview.text_preview) {
        data.textPreview.text_preview = `时间（），批准单位名称（），主要内容（），新单位名称（），隶属关系（）。`
      }
    ...
    handleSave (item, index) {
      item.history_time = this.moment(item.history_time).format('YYYY-MM-DD')
       let list = {
