<template>
  <div class="pd20">
    <Title :title="title" edit :id="modeId" :yearId="yearId"></Title>
    <Card v-for="(item,index) in data" :key="index" class="mt40">
      <Form :ref="`honorFormItem${index}`" :model="item" label-position="left" :label-width="80" :rules="formItemInline">
            ...
            <Row :gutter="32">
                <Col span="8">
                    <Form-item label="身份证号">
                        <Input v-model="item.idCard" :maxlength="50" :disabled="!item.isAdd" @on-change="change" />
                    </Form-item>
                </Col>
                <Col span="8">
            ...
            <Row :gutter="32">
                ...
                <Col span="8">
                    <Form-item label="职务">
                        <Input v-model="item.duty" :maxlength="11" :disabled="!item.isAdd" @on-change="change" />
                    </Form-item>
                </Col>
                <Col span="8">
                    <Form-item label="特殊人群">
                      <Select v-model="item.special" :disabled="!item.isAdd" @on-change="change">
                        <Option v-for="(f,index) in specialList" :value="f.value" :key="index">{{ f.label }}</Option>
                      </Select>
                    </Form-item>
                </Col>
            </Row>
...
export default {
  ...
  methods: {
    init (type = 0) {
        this.$api.post('/member-reversion/employeeRoster/findEmployeeRoster', {
            ...
            if (response.code === 200) {
                if (response.data.employeeRoster_name && response.data.employeeRoster_name !== '') {
                    this.title = response.data.employeeRoster_name
                }
                ...
                if (response.data.employeeRoster.length !== 0) {
                    this.data = []
                    response.data.employeeRoster.forEach((element, index) => {
                        this.data.push({
                            id: element.id,
                            no: element.number,
                            name: element.name,
    ...
    // 保存单个人口信息
    onSave (item, index) {
        this.$api.post('/member-reversion/employeeRoster/saveEmployeeRoster', {
            user_id: this.$user.loginAccount,
            yearId: this.yearId,
            parent_id: this.appId,
            employeeRoster_name: this.title,
            employeeRoster: {
                ...
                sex: item.sex,
                card: item.idCard,
                date_of_birth: this.moment(item.birthday).format('YYYY-MM-DD'),
      ...
      this.$Modal.confirm({
        ...
        onOk:()=>{
            if (item.id !== undefined && item.id !== '') {
                this.$api.post('/member-reversion/employeeRoster/deleteEmployeeRoster', {
                    id: item.id
                }).then(response => {
                    if (response.code === 200) {
    ...
    change () {
        this.preview = ''
        this.preview += `员工总计 ${this.data.length} 人。`
    },
    handleSave () {
  ...
  mounted () {
    this.preview = `编号（），姓名（），性别（），身份证号（），出生年月（），户主（），联系方式（），民族（），党派（），宗教信仰（），职务（），特殊人群（），住址（），（）号。`
  }
}
</script>
