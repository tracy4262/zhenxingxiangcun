<template>
  <div class="vui-upload-component">
    <div class="new-upload-list">
      <div v-for="(item,index) in pictureList" :key="index">
        <template v-if="item.status === 'finished'">
          <p class="t-green pb10">{{item.name}}</p>
        </template>
        <template v-else>
    ...
    <div>
      ...
      >
        <Button type="default" :disabled="disabled" v-if="!preview">
          <Icon type="ios-cloud-upload-outline" size="16" class="pr5"></Icon>{{buttonText}}
        </Button>
      </Upload>
      <!-- <Button type="default" :disabled="disabled" @click="handlePreview" v-if="pictureList.length && onload"><Icon type="ios-eye-outline" size="16" class="pr5"/>预览</Button> -->
      <!-- <Button type="default" :disabled="disabled" @click="handleDownload" v-if="pictureList.length && onload"><Icon type="ios-cloud-download-outline" size="16" class="pr5"/> 下载</Button> -->
      <Button
        class="ml20"
        type="default"
        :disabled="disabled"
        @click="handleRemove"
        v-if="pictureList.length && !preview"
      >
        <Icon type="ios-trash-outline" size="16" class="pr5" />删除
      </Button>
      <p class="t-grey pt5">{{hint}}</p>
    </div>
    <iframe src="about:blank" name="hiddenIframe" class="hide" v-show="showPdf"></iframe>
...
export default {
  ...
  methods: {
    ...
    handleFormatError(file) {
      this.$Notice.warning({
        title: "文件格式有误",
        desc: `文件格式不正确，请选择${this.format.join("/")}格式。`,
        duration: 6
      })
    }
  }
