    ...
    <div class="vui-layout pd20" style="background:#fff">
      ...
      <Row>
        <Col span="18" class="pr20">
          ...
          <item :title="fhabitData.title" :data="fhabitData.data" class="mb50" @on-edit="handleEdit(1)"></item>
          <item :title="fpetsregularData.title" :data="fpetsregularData.data" class="mb50" @on-edit="handleEdit(2)"></item>
          <item :title="fprotectmethodData.title" :data="fprotectmethodData.data" class="mb50" @on-edit="handleEdit(3)"></item>
          <item :title="fmainfeaturesData.title" :data="fmainfeaturesData.data" class="mb50" @on-edit="handleEdit(4)"></item>
          <item :title="fremarksData.title" :data="fremarksData.data" class="mb50" @on-edit="handleEdit(5)"></item>
          <!-- 相关知识资讯政策 -->
          <related></related>
        </Col>
        <Col span="6">
...
export default {
  ...
  methods: {
    // 查询详情
    handlegGetSpecies () {
      this.$api.get('wiki/api/wiki/getSpeciesPest/' + this.indexid).then(response => {
        if (response.code === 200) {
          this.describeData = response.data
          this.speciesid = response.data.speciesid
          // 危害症状
          this.fhabitData.data = response.data.fhabit.replace(/<[^>]*>|&nbsp;/g, '')
          this.$refs['edit'].catalogData[1].data = this.fhabitData.data
          this.$refs['edit'].catalogData[1].fid = response.data.fid
          this.$refs['edit'].catalogData[1].speciesid = response.data.speciesid
          // 发生规律
          this.fpetsregularData.data = response.data.fpetsregular.replace(/<[^>]*>|&nbsp;/g, '')
          this.$refs['edit'].catalogData[2].data = this.fpetsregularData.data
          ...
          // 形态特征
          this.fmainfeaturesData.data = response.data.fmainfeatures.replace(/<[^>]*>|&nbsp;/g, '')
          this.$refs['edit'].catalogData[4].data = this.fmainfeaturesData.data
          this.$refs['edit'].catalogData[4].fid = response.data.fid
          this.$refs['edit'].catalogData[4].speciesid = response.data.speciesid
          // 形态特征
          this.fremarksData.data = response.data.fremarks.replace(/<[^>]*>|&nbsp;/g, '')
          this.$refs['edit'].catalogData[5].data = this.fremarksData.data
