    ...
    handleSave () {
      this.$refs.data.validate((valid) => {
        if (valid) {
          ...
          } else if (this.id === 2) { // 保存发生规律
            list.fpetsregular = this.data.data
          } else if (this.id === 3) { // 保存防治办法
            list.fprotectmethod = this.data.data
          } else if (this.id === 4) { // 保存形态特征
            list.fmainfeatures = this.data.data
          } else if (this.id === 5) { // 保存备注
            list.fremarks = this.data.data
          }
          this.$api.post('wiki/api/wiki/updateSpeciesPest', list).then(response => {
