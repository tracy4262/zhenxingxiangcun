...
public class KnowledgeController {
    ...
    public Result findKnowledge(@PathVariable String page, HttpServletRequest request) {
    ...
    public Result findKnowledgeDetail(HttpServletRequest request) {
        Result result = new Result();
        try{
            String id = request.getParameter("id");
            if (!StringUtils.isEmpty(id)) {
                List<Map<String,Object>> knowledgeDetails = knowledgeService.findKnowLedgeDetail(id);
                Map<String,Object> knowledgeDetail= knowledgeDetails.get(0);
                //遍历视频、音频
                ...
                if(!videoImgs.isEmpty()){
                    knowledgeDetail.put("videoImgs",videoImgs);
                }
                result.setData(knowledgeDetail);
            }
        }catch(Exception e){
            logger.info("知识详情查询出错 -->{}" +e.getMessage());
            result.setCode(500);
        }
        return result;
    }
    ...
    public Result findKnowledgeTitle(@PathVariable String page, HttpServletRequest request) {
        Result result = new Result();
       try{
           ...
           logger.info("/findKnowLedgeTitle，查询参数：{}",info);
           int currentPage = StringUtils.isEmpty(page) ? 0 : Integer.parseInt(page);
           PageInfo<Knowledge> lists = knowledgeService.findKnowLedgeTitle(currentPage, 10, info);
           logger.debug("/findKnowLedgeTitle，查询结果：{}",lists.getList());
           result.setData(lists);
       }catch (Exception e){
          logger.info(" 知识查询：{}",e);
    ...
    public Result findKnowledgeComment(@PathVariable String page, HttpServletRequest request) {
    ...
    public Result findKnowledgeComments(HttpServletRequest request) {
        ...
        map1.put("id", id);
        List<KnowledgeComment> list = knowledgeService.findKnowledgeComments(map1);
        // Integer tote = knowLedgeService.findKnowledgeCommentCount(id);
        // map.put("tote", tote);
        Result result = new Result();
        result.setData(list);
     ...
     * 最新知识
     */
    @RequestMapping(value = "/newknowledge", method = RequestMethod.GET)
    public Result newknowledge() {
        List<KnowledgeDetail> list = knowledgeService.newknowledge();
        Result result = new Result();
        result.setData(list);
        return result;
    }
    ...
    public Result showLed() {
        List<Map<String, Object>> list = knowledgeService.findShow();
        Result result = new Result();
        result.setData(list);
        return result;
    }
    ...
    public Result findMyComment(@PathVariable String page, HttpServletRequest request) {
        ...
        if (!StringUtils.isEmpty(id)) {
            int currentPage = StringUtils.isEmpty(page) ? 0 : Integer.parseInt(page);
            PageInfo<KnowledgeComment> list = knowledgeService.findMyComment(currentPage, 10, report);
            result.setData(list);
        }
        return result;
    }
    ...
    public Result saveZan(HttpServletRequest request) {
        ...
        try {
            ...
            map.put("flag", 1);
            map.put("thumbUpNum", knowledgeComment.getThumbUpNum());
            result.setData(map);
        } catch (Exception e) {
            logger.error(e);
            result.setCode(ResultCode.INTERNAL_SERVER_ERROR);
    ...
    public Result upvote(HttpServletRequest request) {
        ...
        try {
            ...
            map.put("flag", 1);
            map.put("thumbUpNum", knowledgeDetail.getThumbUpNum());
            result.setData(map);
        } catch (Exception e) {
            logger.error(e);
            result.setCode(ResultCode.INTERNAL_SERVER_ERROR);
    ...
    public Result publish(@RequestBody Map<String, Object> params, HttpServletRequest request) {
        ...
        try {
            ...
            int num = knowledgeService.insertKnowledgeDetail(detail);
            ...
            knowledge.setLabel(params.get("custom").toString());
            knowledge.setInformationDetailId(detail.getId());
            String doctype=(String)params.get("leixin");
            String content=(String)params.get("content");
            if("图册".equals(doctype)){
                knowledge.setImageAdd(content.substring(0,content.indexOf(">")+1));
            }
            if("视频".equals(doctype)){
                knowledge.setImageAdd(videoImgs.get(0).getAddr());
            }
            knowledge.setColumnType(doctype);
            knowledge.setTitle(params.get("title").toString()); //咨询标题
            knowledge.setCreateTime(detail.getCreateTime());
            knowledge.setCommentNum(detail.getBrowseNumber()); //评论次数
            knowledge.setLabel(params.get("custom").toString()); //标签 关键字
            knowledge.setDocType(params.get("category").toString());
            knowledge.setImageAdd((String)params.get("imageAdd"));
            if(StringUtils.hasText((String)params.get("species"))){
                knowledge.setSpeciesName((String)params.get("species"));
            }
            if(StringUtils.hasText(params.get("speciesId").toString()))// 物种名称
                knowledge.setSpeciesId(params.get("speciesId").toString());
            if(StringUtils.hasText((String)params.get("goodsname"))){
                knowledge.setProduct((String)params.get("goodsname"));
            }
            if(StringUtils.hasText((String)params.get("servicename"))){
                knowledge.setService((String)params.get("servicename"));
            }
            if(StringUtils.hasText((String)params.get("industry"))){
                knowledge.setIndustrialId((String)params.get("industry"));//行业分类
            }
            knowledge.setAllRange((String)params.get("district"));
            knowledge.setFriendReminding((String)params.get("tixing"));
            knowledge.setRemindingContent((String)params.get("con"));
            knowledge.setIsSms("是".equals((String)params.get("sms")) ? 0 : 1 );
            knowledge.setIsPublish((Integer) params.get("security"));
            knowledge.setComponsation((String)params.get("money"));
            knowledge.setChapter((String)params.get("chapter"));

            Object abstracts = params.get("summary");
            if (null != abstracts) {
                knowledge.setAbstracts(abstracts.toString());
            }
            Object classifiedId = params.get("type");
            if (null != classifiedId) {
                knowledge.setClassifiedId(classifiedId.toString());
            }
            String xianshi = params.get("xianshi").toString();
            if (!"是".equals(xianshi)) {
                knowledge.setIsToPage(1);
            }else{
                knowledge.setIsToPage(0);
            }
            knowledge.setAuthor(params.get("account").toString());
            ...
            if (MapUtils.getString(params, "isBook") != null) {
                ...
                bookMap.put("bookType", "knowledge");
                ...
                if (bookDetails != null && bookDetails.size()>0) {
                    ...
                    for (int i = 0; i< ChapterSize ; i++ ) {
                        ...
                        saveChapterInfo.put("bookType", "knowledge");
                        ...
                        for (int j=0; j<partSize; j++) {
                            ...
                            savePartInfo.put("bookType", "knowledge");
                            saveBookDetailList.add(savePartInfo);

                        }
                    }
                    //批量插入图书信息,先删除，后插入
                    informationBookDetailService.batchInsert(saveBookDetailList, detail.getId(), "knowledge");
    ...
    public Result replyList(@RequestBody Map<String, Object> params, HttpServletRequest request) {
        ...
        try {
            ...
            PageInfo<KnowledgeComment> pageInfo = knowledgeService.replylist(currentPage, 10, map);
    ...
    public Result replyChildrenList(@RequestBody Map<String, Object> params, HttpServletRequest request) {
        ...
        try {
            ...
            PageInfo<KnowledgeComment> pageInfo = knowledgeService.replyToOther(currentPage, 10, postid);
    ...
    public Result getBriefNews() {
        Result result = new Result();
        try {
            List<Map<String, Object>> list = knowledgeService.getBriefNews();
            result.setData(list);
    ...
    public Result upAndNextPage(@PathVariable Integer id) {
        ...
        Result result = new Result();
        try {
            Map<String, Object> map = knowledgeService.findUpAndNextPage(id);
            result.setData(map);
            result.setCode(ResultCode.SUCCESS);
