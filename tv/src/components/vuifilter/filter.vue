    ...
    <section class="vui-high-filter">
        ...
        <Row type="flex" justify="center" align="middle" class="vui-high-filter-letter">
          ...
          <Col span="13">
            <Row class="vui-filter-search">
              ...
              <Col span="4">
                <Button  long @click="handleSearch"><Icon type="android-search"></Icon></Button>
              </Col>
            </Row>
          </Col>
        </Row>
      ...
      <div @click="classifyShow = !classifyShow" v-if="!simple">
        <div class="vui-high-filter-item" v-if="selClassifyText.length && !classifyShow">
          当前选中分类：<span v-for="item in selClassifyText" class="sel-text">{{item.label}}&emsp;</span>
        </div>
        <template v-else>
      ...
      <div class="vui-high-filter-classify" v-if="classifyShow">
        ...
        <div class="tc pd10" style="border-top:1px solid #d7dde4" v-if="!simple">
          <Button type="primary" class="mr10" @click.native="handleSaveClassify">确定</Button>
          <Button  @click.natiev="classifyShow = !classifyShow">取消</Button>
        </div>
      </div>
      ...
      <div class="vui-high-filter-product scroll" v-if="!classifyShow">
        <Row v-if="resultDatas.length">
          <template v-for="(item,index) in resultDatas">
            <Col span="6" :class="{'on':item.checked}" @click.native="handleSelResult(item,index)">
              <div class="vui-high-filter-product-item ell">{{item.label}}</div>
            </Col>
          </template>
        </Row>
    ...
    <template slot="footer">
      <Row class="pd10 tl">
        ...
        <Col span="6" :offset="pageShow ? '0' : '18'">
          ...
          <Button  @click="highFilterShow = false">取消</Button>
        </Col>
      </Row>
    </template>
  </Modal>
