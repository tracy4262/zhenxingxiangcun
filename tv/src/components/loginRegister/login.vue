  ...
  <div class="new-login">
    <Form ref="loginData" :model="loginData" :rules="ruleInline">
          ...
          <Form-item prop="password">
            <i-input type="password" v-show="show" size="large" v-model.trim="password" @on-focus="onFocus" placeholder="请输入密码" @keyup.enter.native="login">
              <span slot="prepend" class="pr10 pl10">
                <Icon type="ios-locked-outline" class="pr10" size="18"></Icon>密码
              </span>
            </i-input>
              <i-input type="password" ref="password" v-show="!show" size="large" @on-blur="onBlur" v-model.trim="loginData.password" placeholder="请输入密码" @keyup.enter.native="login">
                 <span slot="prepend" class="pr10 pl10">
                  <Icon type="ios-locked-outline" class="pr10" size="18"></Icon>密码
                </span>
              </i-input>
          ...
          <!-- <Form-item prop="password">
              <i-input type="password" v-show="show" size="large" @on-focus="onFocus" v-model.trim="password" placeholder="请输入密码" @keyup.enter.native="login">
                  <span slot="prepend" class="pr10 pl10">
          ...
          <Form-item>
              <Row>
                  <Col span="12">
                      <Checkbox v-model="loginData.remberPassWord">记住密码</Checkbox>
                  </Col>
...
import sha1 from '../../js/sha1.js'
  ...
  export default {
      ...
      return {
          ...
          ruleInline: {
              user: [
                  ...
                  { min:4, message:'用户名为4-16个字符组成，包含字母、数字' }
              ],
              isSuccess: [
              ...
              password: [
                  ...
                  { type: 'string', min: 6, message: '登录密码为6-16个字符组成，区分大小写', trigger: 'blur' },
                  { validator: password, message:'登录密码为6-16个字符组成，区分大小写' }
              ]
          }
      }
    },
    ...
    methods: {
        ...
        // 登录
        toLogin (data) {
          sessionStorage.removeItem('ns51pwd')
          sessionStorage.setItem('ns51pwd', JSON.stringify(this.loginData))
          this.$api.post('/member/login/login',data).then(response => {
                if (response.data) {
                    ...
                    this.$api.post('/member/login/login', data ).then(response => {
                        if (response.data) {
                            ...
                            if (response.data.userType == 10) {
                                ...
                                window.location.href = `${this.$url.toNswy}user-auth-admin/auth/step1`
                            } else {
