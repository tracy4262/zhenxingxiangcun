...
@RestController
@RequestMapping("/traceSecurity")
public class TraceSecurityController {

    private static final Logger logger = LogManager.getLogger(TraceSecurityController.class);
    ...
    public Result save(@RequestBody Map<String,Object> params) {
        ...
        try {
            ...
            mongoTemplate.remove(deleteQuery, Map.class, "commodity_traceability_info");
            ...
            if(MapUtils.isNotEmpty(info)) {
                ...
                mongoTemplate.save(info, "commodity_traceability_info");
            }
    ...
    public Result find(@RequestBody(required=false) Map<String,Object> params) {
        ...
        try {
            ...
            List<Map> info = mongoTemplate.find(query, Map.class, "commodity_traceability_info");
            if(CollectionUtils.isNotEmpty(info)) {
                map.put("info", info.get(0));
            } else {
                if(MapUtils.getString(params,"templateType").equals("1")) {
                    info = mongoTemplate.find(queryTwo, Map.class, "commodity_traceability_info");
                    if (CollectionUtils.isNotEmpty(info)) {
                        map.put("info", info.get(0));
                    }
                }
