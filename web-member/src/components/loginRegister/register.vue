        ...
        <Form-item prop="city" class="mb30 loaction-vui-cascander">
            <i-input :type="typep2" size="large">
                ...
                <div class="vui-cascander">
                    <vui-cascander isCheckedCounty :vlue="registerData.city" @handle-get-result="handleGetData" :size="'large'">
                    </vui-cascander>
                </div>
        </Form-item>
  ...
  export default {
    ...
    data () {
        ...
        const userName = (rule, value, callback) => {
            ...
            if (!uPattern.test(value)) {
                return callback(new Error('用户名为4-16位，可使用字母、数字'));
            }else {
                callback()
            }
        }
        const password = (rule, value, callback) => {
            ...
            if (!uPattern.test(value)) {
                return callback(new Error('登录密码为6-16位，可使用字母、数字'));
            }else {
                callback()
            }
        }
        ...
        return {
            ...
            ruleInline1: {
                username: [
                    ...
                    { min:4, message:'用户名为4-16个字符组成，可使用字母、数字' },
                    { validator: userName, },
    ...
    created() {
      ...
      window.callback = function(res){
        ...
        if(res.ret === 0){
          ...
          that.$refs['registerData'].validate()
        }
        if(res.ret === 1){
    ...
    methods: {
        ...
        handleSubmit () {
            ...
            // }
            // 代理注册的时候要往里面添加代理人的账号
            if (this.$user && this.$user.loginAccount && this.$user.loginAccount !== '') {
                this.registerData.proxyAccount = this.$user.loginAccount
            }
