        ...
        <FormItem label="选项参数" prop="list">
          <Row :key="index" v-for="(item, index) in data.list">
            <Col span="10">
              ...
              v-model="item.value"
              :maxlength="12"
              :class="{active: data.value === item.value}"
              ></Input>
            </Col>
            <Col span="2" class="tr">
            ...
            <!-- <Col span="2" class="tr">
              <Tooltip content="设为默认" placement="left">
                <Button
                type="text"
                size="small"
                shape="circle"
                icon="checkmark-round"
                :class="{active: data.value === index.toString()}"
                @click="handleSetDefault(item.value)"
                ></Button>
              </Tooltip>
            </Col> -->
          </Row>
...
export default {
  ...
  methods: {
    ...
    handleAddOption () {
      this.data.list.push({
        value: `默认选项${this.data.list.length}`
      })
      this.data.value = this.data.list[0].value
    },
    handleDel (index) {
