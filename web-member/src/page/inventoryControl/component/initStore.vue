...
<div>
  <Form ref="info" :model="info" label-position="right" :label-width="100" :rules="ruleInline">
    ...
    <Row :gutter="32" class="pl20 pr20">
      <Col span="12">
        <FormItem label="数量" prop="number">
          <InputNumber :max="100000000" :min="0.01" :step="1" :precision="2" :active-change="false" v-model="info.number" style="width: 100%;" @on-change="change"></InputNumber>
        </FormItem>
      </Col>
      <Col span="12">
    ...
    <Row :gutter="32" class="pl20 pr20">
      <Col span="12">
        <FormItem label="单价" prop="price">
          <InputNumber :max="100000000" :min="0.01" :step="1" :precision="2" :active-change="false" v-model="info.price" style="width: 95%;" @on-change="change"></InputNumber>&nbsp;&nbsp;元
        </FormItem>
      </Col>
      <Col span="12">
    ...
    <Row :gutter="32" class="pl20 pr20">
      ...
      <Col span="12">
        <FormItem label="入库类型" prop="inStoreTypeName">
          <Input v-model="info.inStoreTypeName" readonly />
        </FormItem>
      </Col>
    </Row>
    <Row :gutter="32" class="pl20 pr20">
  ...
  <div class="tc mt20">
    ...
    <!-- <Button type="success" class="ml10" @click="saveStore(1)">保存并录入下一条</Button> -->
  </div>
</div>   
</template>
...
export default {
  data () {
    return {
      ...
      info: {
        ...
        inStore: '', // id
        customName: '',
        inStoreType: 'd6ac4bcfdfa1416c9532fedbcf304e55', // id
        inStoreTypeName: '库存初始化', // name
        operatorAccount: '',
        createTime: ''
      },
      ruleInline: {
        ...
        totalPrice: [
          { required: true, type: 'string', message: '请填写产品合计', trigger: 'change' }
        ],
        batchNumber: [
        ...
        inStoreTypeName: [
          { required: true, type: 'string', message: '请填写入库类型', trigger: 'change' }
        ],
        operatorAccount: [
  ...
  methods: {
    ...
    change () {
      this.info.totalPrice = (this.info.price * this.info.number).toFixed(2)
    },
    saveStore (type) {
        ...
        if (valid) {
          ...
          this.$api.post('/shop/inventory/basicSetting/enterStoreSave', this.info).then(response => {
            ...
            if (response.code === 200) {
              ...
              if (type === 0) {
                // response.data 返回入库单号
                this.$emit('close', '1', response.data)
              } else {
                this.initAdd()
              }
