...
export default {
  ...
  methods: {
    init () {
      this.$api.post('/shop/pushShopInfo/findPushCommitment', {
    ...
    next () {
      ...
      this.$api.post('/shop/pushShopInfo/savePushCommitment', list).then(response => {
        console.log('response', response)
        if (response.code === 200) {
          this.$Message.success('商品发布成功！')
          this.$router.push('/goods/goods-shelf')
        }
      }).catch(error => {
