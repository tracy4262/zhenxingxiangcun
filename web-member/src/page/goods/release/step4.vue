  ...
  methods: {
    getProductionBase () {
      this.$api.post('/member-reversion/productionBase/list', {
        ...
        console.log('productionBase', res)
        if (res.code === 200) {
          if (res.data.list.length > 0) {
            res.data.list.forEach(element => {
              this.productionBaseList.push({
                label: element.productionBaseName,
                value: element.id
              })
            })
          }
    ...
    init () {
      this.$api.post('/shop/pushShopInfo/findPushTrace', {
        account: this.$user.loginAccount,
    ...
    save () {
      ...
      this.$api.post('/shop/pushShopInfo/savePushTrace', list).then(response => {
