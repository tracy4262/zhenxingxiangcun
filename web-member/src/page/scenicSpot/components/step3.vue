  ...
  <div>
    <Row class="pb10">
      ...
      <Col span="12" class="tr">
        <a :href="`/scenicSpot/add-set-meal?id=${id}`">
...
export default {
            ...
            return h('div', [
              h(
                'Button',
                {
                  ...
                  on: {
                    click: () => {
                      this.$router.push(`/scenicSpot/add-set-meal?id=${this.id}&activeId=${params.row.setMealId}`)
                    }
                  }
                },
                '编辑'
  ...
  methods: {
    ...
    // 初始化查询
    handleInit () {
      this.$api.post('/member/fishing/findFishingService', {pageNum: this.pageNum, type: '2', pageSize: this.pageSize,account: this.account,id: this.id}).then(response => {
    ...
    // 上一步
    handleBack() {
      this.$router.push('/scenicSpotAddService/step2?id=' + this.id)
    },
    // 下一步
    handleSave() {
      this.$router.push('/scenicSpotAddService/step4?id=' + this.id)
    },
    // 以后在完善
    handleNext() {
      this.$router.push('/scenicSpot/service')
    }
  }
}
</script>
<style lang>
.ivu-table-row i.ivu-icon-ios-arrow-right {
