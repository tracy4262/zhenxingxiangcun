        ...
        <Row>
          ...
          <Col span="17">
            ...
            <Title title="截止日期" class="mt30"></Title>
              <FormItem label="截止日期" prop="endDate" class="mt20 mb40">
            ...
            <Row type="flex" align="middle" style="background: #F3F3F3;" class="pd10 tr">
              ...
              <Col span="6" offset="2">
                <a :href="`/scenicSpotAddService/step3?id=${id}`">
...
<script>
import {numAdd, numMulti, isNumber1} from '~utils/utils'
...
export default {
            ...
            return h('InputNumber', {
              ...
              on: {
                'on-change': (val) => {
                  ...
                  this.checkData[params.index].total = parseFloat((numMulti(Number(params.row.price), val)).toFixed(2))
                  this.countFn()
                }
              }
            })
            ...
            return h('Button', {
              ...
              on: {
                click: () => {
                  ...
                  this.countFn()
                  if (this.checkData.length === 0) this.totalPrice = 0
                }
              }
          }, '删除')}
        }
  ...
  created() {
    ...
    // // 初始化获取数据
    this.$api.post('/member/fishing/findProductService', {account: this.account, setMealId: this.activeId, type: '2', fishServiceId: this.id, status: '1'}).then(response => {
      if (response.code === 200) {
        this.setMealData = response.data[0].leftProduct
        this.setMealData.forEach(e => {
          e.total = parseFloat(e.ticketPrice).toFixed(2)
          e.managementId = e.id
          e.name = e.ticketName
          e.account = this.account
          e.price = parseFloat(e.ticketPrice).toFixed(2)
          e.num = 1
        })
        if (this.activeId) {
          ...
          this.form.setMealPrice = response.data[0].setMealPrice ? Number(response.data[0].setMealPrice) : null
          this.form.payType = response.data[0].payType
          this.form.money = response.data[0].money ? Number(response.data[0].money) : null
          this.form.endDate = response.data[0].endDate
          this.$refs['setMeal'].handleInit(response.data[0].selectedProduct)
          this.totalPrice = response.data[0].totalPrice ? Number(response.data[0].totalPrice) : null
        }
      }
    })
  },
  methods: {
    ...
    handleSave () {
      ...
      this.$refs['setMealName'].validate((valid) => {
        ...
        if (valid) {
          let list = Object.assign({account: this.account, type: '2', fishServiceId: this.id,}, this.form)
          // flag 0保存，1，更新
          if (this.activeId) {
          ...
          this.$api.post('/member/fishing/saveProduct', list).then(response => {
            if (response.code === 200) {
              ...
              this.$router.push(`/scenicSpotAddService/step3?id=${this.id}`)
            } else {
