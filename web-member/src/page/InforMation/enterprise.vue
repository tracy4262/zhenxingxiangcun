<template>
	<div class="mt40">
		<div class="enterprise-card ml10 mr10 mb20" v-for="item in enterprise" :key="item.id" v-if="enterprise.length > 0">
			<router-link :to="{path:'../companyGate/index',query: {uid: item.loginAccount}}">
				<Col span="6">
					<Avatar class="ml5 mt5" size="large" :src="item.logoUrl" />
				</Col>
				<Col span="18">
					<Row>
						<span class="ell enterprise-name" :title="item.corpName">{{ item.corpName }}</span>
					</Row>
					<Row class="enterprise-addr mt10">
						<Col span="12"><span>湖北武汉</span></Col>
						<Col span="12" class="tr"><span>法人：李洋</span></Col>
					</Row>
					<Row class="mt10">
		...
		<div class="tc mt30 mb60" v-if="enterprise.length > 0">
...
export default {
	...
	methods: {
		show () {
			this.$api.post('/member/corpInfo/findCorpInfoTitle/1', {
				...
				if (response.code === 200) {
					this.enterprise = response.data.list.slice(0, 9)
				}
			}).catch(error => {
		...
		more () {
            this.$router.push('/51index/enterpriseList')
        },
	}
}
</script>
