<template>
    <div class="pd20" style="min-height: 500px;">
        <Button :type="activeIndex === 0 ? 'primary' : 'text'" @click="find">查找产品</Button>
        <Button :type="activeIndex === 1 ? 'primary' : 'text'" @click="has">已推荐产品</Button>
        <Form ref="queryInfo" :model="queryInfo" label-position="left" :label-width="100" :rules="ruleInline" class="mt20">
            <Row :gutter="32">
                ...
                <Col span="7">
                    <Form-item prop="speci" class="input" label="产品名称">
            ...
            <CheckboxGroup v-model="choosed">
                <Row :gutter="24">
                    <Col span="6" v-for="(item, index) in list" class="mb20">
                        ...
                        <product-item @refresh="refresh" :item="item"></product-item>
                        <!-- 咨询服务 -->
    ...
    methods: {
        init () {
            ...
            this.$api.post('/member-reversion/myRecommend/productList', {
                ...
                productLocation: this.queryInfo.location,
        ...
        add () {
            if (this.choosed.length === 0) {
                this.$Message.info(this.activeIndex === 0 ? '请先选择要推荐的产品！' : '请先选择要取消推荐的产品！')
                return
            }
            this.$Modal.confirm({
                title: '操作提示',
                content: this.activeIndex === 0 ? '设置为推荐的产品将在您的门户对外宣传展示！请确认是否设置为推荐产品！' : '取消推荐的产品将从您的门户删除！请确认是否取消推荐！',
                onOk: () => {
                    // 请求添加接口，请求取消接口
                    // 刷新当前页面
                    let list = []
