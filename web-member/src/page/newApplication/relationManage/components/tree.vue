<template>
  <div class="relation-manage">
    <p class="tree-title"><Icon type="ios-people-outline" size="24" class="mr10"/>好友分组</p>
...
export default {
  ...
  methods: {
    ...
    onSaveTitle () {
      ...
      this.$api.post('/member/relationshipCircle/saveOrUpdateGroup', data).then(response => {
    ...
    remove (root, node, data) {
      ...
      if(friendList != 0 && childrenGroupList != 0){
        ...
        this.$Modal.confirm({
            ...
            cancelText: '取消',
            onOk: () => {
              this.$api.post('/member/relationshipCircle/deleteGroup', {id: data.id}).then(response => {
                if (response.code === 200) {
