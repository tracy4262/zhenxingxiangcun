<template>
  <div style="position: relative;">
    ...
    <div class="pr20 pl20 pt20">
      <friendList focusType="1" :data="data" @on-change="nextPage" @on-focus="handleFocus" :pages="pages"  @on-init="nextPage"></friendList>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    getInit () {
      ...
      this.$api.post('/member/relationshipCircle/findLoginByCircle', this.form).then(response => {
    ...
    handleFocus (arr) {
      ...
      // this.$refs['groupList'].isShow = true
    },
    // 查询
    search () {
      this.nextPage(1)
