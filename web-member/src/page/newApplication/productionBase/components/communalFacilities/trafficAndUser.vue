                ...
                <Col span="24">
                    <Form-item label="上传图片">
                        <vui-upload
                            ...
                            :size="[80,80]"
                        ></vui-upload>
                        <div v-show="!item.isAdd" v-for="(pic, i) in item.pictureList" :key="i">
                            <img :src="pic" style="width: 80px; height: 80px; float: left; margin-right: 10px;">
                        </div>
                    </Form-item>
                </Col>
            </Row>
            <Card v-for="(item2, index2) in item.siteInfo" class="mt40" :key="j">
                <Row>
                    <Col span="12" offset="12" class="tr mb20">
                        <div class="btn-toolbar" style="margin-top: -10px;">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 初始化加载数据
            init (type = 0) {
                this.$api.post('/member-reversion/productionBase/communalFacilities/findTrafficOfUserInfo', {
                    account: this.$user.loginAccount,
                ...
                this.$Modal.confirm({
                    ...
                    onOk:()=>{
                        if (item.id !== undefined && item.id !== '') {
                            this.$api.post('/member-reversion/productionBase/communalFacilities/deleteTrafficOfUserInfo', {
                                id: item.id
                            }).then(response => {
            ...
            save (item, index) {
                ...
                this.$api.post('/member-reversion/productionBase/communalFacilities/saveTrafficOfUserInfo', data).then(response => {
