  ...
  <div style="background:#f9f9f9;" class="mb40">
   <!-- 第一产业 -->
        <div class="pd20" >
          ...
          <Form :label-width="60" label-position="left" :ref="`data${index}`" v-for="(item, index) in data" :model="item" :rules="dataLine"  :key="index">
            <Row  :gutter="16">
              <Col span="8">
                <FormItem label="产品名称">
                  <!-- 从通用商品名中选择 -->
    ...
    methods: {
      ...
      handleDel (item, index) {
        this.$Modal.confirm({
            ...
            onOk:()=>{
              if (item.id) {
                  this.$api.post('/member-reversion/productionBase/ecoSocial/deleteTechnicalProduct', {id: item.id}).then(response => {
      ...
      handleSave () {
        ...
        if (flag) {
          ...
          this.$api.post('/member-reversion/productionBase/ecoSocial/saveTechnicalProduct', list).then(response => {
              if (response.code === 200) {
