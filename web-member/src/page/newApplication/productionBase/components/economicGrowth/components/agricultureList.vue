  ...
  <div style="background:#f9f9f9;" class="mb40">
   <!-- 第一产业 -->
        <div class="pd20" >
          ...
          <Form :label-width="80" label-position="left"  :ref="`data${index}`" v-for="(item, index) in data" :model="item":rules="dataLine" :key="index">
            <Row  :gutter="32">
              <Col span="8">
                <FormItem label="农产品名称">
                  <!-- 从通用商品名中选择 -->
                  <vui-product-name :ref="`vpn${index}`" :disabled="!edit" :values="item.productTypeName" @handle-get-result="handleResult($event, item, index)"></vui-product-name>
                </FormItem>
              </Col>
              <Col span="8">
              ...
              <!-- <Col span="3">
               <Select v-model="data.YieldUnit" filterable  :disabled="!edit" placeholder="请选择单位">
                  <Option v-for="item in units" :value="item.unit_name" :key="item.unit_name">{{ item.unit_name }}</Option>
              </Select> -->
                <!-- <Input placeholder="请选择单位" v-model="item.YieldUnit" :disabled="!edit"></Input> -->
              <!-- </Col> -->
              <Col span="8">
    ...
    methods: {
      ...
      handleDel (item, index) {
        this.$Modal.confirm({
            ...
            onOk:()=>{
              if (item.id) {
                  this.$api.post('/member-reversion/productionBase/ecoSocial/deleteFarmProduct', {id: item.id}).then(response => {
      ...
      handleSave () {
        ...
        if (flag) {
          ...
          this.$api.post('/member-reversion/productionBase/ecoSocial/saveFarmProduct', list).then(response => {
              if (response.code === 200) {
