...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 初始化页面时加载数据
            init () {
                this.$api.post('/member-reversion/productionBase/envCondition/findEcologyEnvQua', {
                    account: this.$user.loginAccount,
                    baseId: this.baseId,
                    dictId: this.modeId
                }).then(response => {
            ...
            handleSave () {
                ...
                this.$api.post('/member-reversion/productionBase/envCondition/modifyEcologyEnvQua', data).then(response => {
                    if (response.code === 200) {
