      ...
      <Col span="20">
        <Card :padding="0">
          <div class="pd20" style="border-bottom: 1px solid #f5f5f5;">
            ...
            @on-edit="handleEdit"
            @on-del="handleDel"
            @on-cancel="handleCancels"></serviceSearch>
          </div>
          <div class="pd30">
            <components-list
              path="addService"
              ...
              <!-- <serviceList
                  ...
                  @on-add="addCollection"
                  @on-init="pageChange"
                  @on-cancel="handleCancel" ></serviceList> -->
          </div>
        </Card>
      </Col>
    </Row>
    <vuiCommodity ref="vuiCommodity" @on-save="onSaveFocus" :input="false" type="2" :num="1000"></vuiCommodity>
  </div>
</template>
<script>
    ...
    methods: {
      ...
      onSaveFocus (e) {
        ...
        if (e.length) {
          let arr = []
          e.forEach(e => {
            e.serviceId = e.commodityId
            e.serviceName = e.commodityName
          })
          let data = {
            account: this.$user.loginAccount,
            type: this.types,
