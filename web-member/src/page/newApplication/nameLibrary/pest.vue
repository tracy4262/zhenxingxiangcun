<template>
  <div>
    ...
    <!-- type 1 品种 2 病害 3 虫害 -->
    <vuiVariety :input="false" ref="vuiVariety" @on-save="onSaveFocus" :num="1000" type="3"></vuiVariety>
  </div>
</template>
<script>
...
import vuiVariety from '~components/vui-variety'
  export default {
    ...
    methods: {
      ...
      // 删除调用的统一接口
      dels (data) {
        this.$api.post('/wiki/api/wiki/deletePestSpecies', {dataList: data, type: this.types, account:this.$user.loginAccount}).then(response => {
          if (response.code === 200) {
