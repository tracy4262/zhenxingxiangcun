  ...
  <div>
    <ul class="new-name-library">
      ...
      <li :class="type === '0' ? 'collection' : 'add'" v-for="(item, index) in data">
        ...
        <div class="check-box" :class="item.check ? 'isCheck' : ''" v-if="edit && type === '1' && item.aduitStatus === '审核未通过'" @click="handleCheck(item, index)">
          <Icon type="md-checkmark" />
        </div>
        <!-- type 0 收藏  1新增 -->
        ...
        <div class="conent"  :class="item.check && edit ? 'conentBorder' : ''" v-if="type === '1' ">
          <div v-if="item.image" style="height: 118px;">
            <img src="../../../../../static/img/goods-list-no-picture1.png" alt="" v-if="!item.image.length" width="100%" height="100%">
            <img :src="item.image[0] ? item.image[0] : '../../../../../static/img/goods-list-no-picture1.png'" alt="" v-else width="100%" height="100%">
          </div>
          <div class="cover" v-if="!edit && item.aduitStatus == '审核未通过'" @click="handleCancels(item, index)">
            删除
          </div>
        </div>
        <p class="tc name ell" v-if="item.commodityName" @click="detail(item)">{{item.commodityName}}</p>
        <p class="tc name ell" v-if="item.serviceName" @click="detail(item)">{{item.serviceName}}</p>
        <p class="tc status">
          <!-- aduitStatus 待审核  审核通过 审核未通过 -->
          <!-- 待审核：不能编辑 不能删除    审核通过：不能删除，可以编辑  审核未通过： 可以删除，可以编辑-->
          <span v-if="item.aduitStatus == '审核未通过'" class="t-red">未通过</span>
          <span v-if="item.aduitStatus == '待审核'" class="t-orange">审核中</span>
          <span v-if="item.aduitStatus =='审核通过'" class="t-grey">已通过</span>
        </p>
      </li>
    </ul>
  ...
  export default {
    ...
    methods: {
      ...
      detail(item) {
        // <!-- type 0 我收藏的 1 我新增的 -->
        let e = ''
        if (this.type === '1') { // 新增的查询用id 
          e = item.id
        } else { // 收藏的查询 通用商品名 commodityId  通用服务名 serviceId
          e = item.serviceId ? item.serviceId : item.commodityId
        }
        //  <!-- 待审核：不能编辑 不能删除    审核通过：不能删除，可以编辑  审核未通过： 可以删除，可以编辑-->
