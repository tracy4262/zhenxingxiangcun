...
<script>
    ...
    export default {
        ...
        methods: {
            // 点击更新数据
            handleUpdate () {
                this.$api.post('/member-reversion/communalFacilities/updateGisZlsss',{
            ...
            // 初始化加载数据
            init (type = 0) {
                this.$api.post('/member-reversion/communalFacilities/findTapWaternFacilitiesInfo', {
                    account: this.$user.loginAccount,
                    yearId: this.yearId,
                    ...
                    if (response.code === 200) {
                        ...
                        // if (response.data.tapWaternFacilitiesInfoName && response.data.tapWaternFacilitiesInfoName !== '') {
                        //     this.title = response.data.tapWaternFacilitiesInfoName
                        // }
                        ...
                        this.data = []
                        if (response.data.tapWaternFacilitiesInfo.length !== 0) {
                            response.data.tapWaternFacilitiesInfo.forEach((element, index) => {
                                let no
                                if (index === 0) {
                                    no = '01'
                                } else {
                ...
                this.$Modal.confirm({
                    ...
                    onOk:()=>{
                        if (item.id !== undefined && item.id !== '') {
                            this.$api.post('/member-reversion/communalFacilities/deleteTapWaternFacilitiesInfo', {
                                id: item.id
                            }).then(response => {
            ...
            save (item, index) {
                if (!this.saveisloading) {
                    ...
                    let data = {
                        ...
                        yearId: this.yearId,
                        parentId: this.modeId,
                        tapWaternFacilitiesInfoName: this.title,
                        templateId: this.templateId,
                        tapWaternFacilitiesInfo: {
                    ...
                    this.$api.post('/member-reversion/communalFacilities/saveTapWaternFacilitiesInfo', data).then(response => {
            ...
            change () {
                this.preview = ''
                this.preview += `自来水设施有`
                let runningWaterNum = 0
                let otherNum = 0
                this.data.forEach(element => {
                    if (element.name && element.name !== '') {
                        this.preview += `${element.name}、`
                    }
                    if (element.commonName === '自来水厂' && element.userNumber !== '') {
                        runningWaterNum += parseInt(element.userNumber)
                ...
                this.preview += `其中自来水用户${runningWaterNum}户，其他用户${otherNum}户。`
            },
            leftRefresh () {
