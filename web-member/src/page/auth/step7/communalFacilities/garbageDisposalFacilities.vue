                ...
                <Col span="8">
                    <Form-item label="通用名称">
                        <Select v-model="item.commonName" :disabled="!item.isAdd" @on-change="change">
                            <Option value="垃圾中转站">垃圾中转站</Option>
...
<script>
    ...
    export default {
        ...
        methods: {
            // 点击更新数据
            handleUpdate () {
                this.$api.post('/member-reversion/communalFacilities/updateGisLgcl',{
            ...
            // 初始化加载数据
            init (type = 0) {
                this.$api.post('/member-reversion/communalFacilities/findGarbageDisposaFacilitiesInfo', {
                    account: this.$user.loginAccount,
                    yearId: this.yearId,
                    ...
                    if (response.code === 200) {
                        ...
                        // if (response.data.garbageDisposaFacilitiesInfoName && response.data.garbageDisposaFacilitiesInfoName !== '') {
                        //     this.title = response.data.garbageDisposaFacilitiesInfoName
                        // }
                        ...
                        this.data = []
                        if (response.data.garbageDisposaFacilitiesInfo.length !== 0) {
                            response.data.garbageDisposaFacilitiesInfo.forEach((element, index) => {
                                let no
                                if (index === 0) {
                                    no = '01'
                                } else {
                ...
                this.$Modal.confirm({
                    ...
                    onOk:()=>{
                        if (item.id !== undefined && item.id !== '') {
                            this.$api.post('/member-reversion/communalFacilities/deleteGarbageDisposaFacilitiesInfo', {
                                id: item.id
                            }).then(response => {
            ...
            save (item, index) {
                if (!this.saveisloading) {
                    ...
                    let data = {
                        ...
                        yearId: this.yearId,
                        parentId: this.modeId,
                        garbageDisposaFacilitiesInfoName: this.title,
                        templateId: this.templateId,
                        garbageDisposaFacilitiesInfo: {
                    ...
                    this.$api.post('/member-reversion/communalFacilities/saveGarbageDisposaFacilitiesInfo', data).then(response => {
