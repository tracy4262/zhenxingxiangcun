...
<script>
    ...
    export default {
        ...
        methods: {
            // 点击更新数据
            handleUpdate () {
                this.$api.post('/member-reversion/communalFacilities/updateAbss',{
            ...
            // 初始化加载数据
            init (type = 0) {
                this.$api.post('/member-reversion/communalFacilities/findSecurityFacilitiesInfo', {
                    account: this.$user.loginAccount,
                    yearId: this.yearId,
                    ...
                    if (response.code === 200) {
                        ...
                        // if (response.data.securityFacilitiesInfoName && response.data.securityFacilitiesInfoName !== '') {
                        //     this.title = response.data.securityFacilitiesInfoName
                        // }
                        ...
                        this.data = []
                        if (response.data.securityFacilitiesInfo.length !== 0) {
                            response.data.securityFacilitiesInfo.forEach((element, index) => {
                                let no
                                if (index === 0) {
                                    no = '01'
                                } else {
                ...
                this.$Modal.confirm({
                    ...
                    onOk:()=>{
                        if (item.id !== undefined && item.id !== '') {
                            this.$api.post('/member-reversion/communalFacilities/deleteSecurityFacilitiesInfo', {
                                id: item.id
                            }).then(response => {
            ...
            save (item, index) {
                if (!this.saveisloading) {
                    ...
                    let data = {
                        ...
                        yearId: this.yearId,
                        parentId: this.modeId,
                        securityFacilitiesInfoName: this.title,
                        templateId: this.templateId,
                        securityFacilitiesInfo: {
                    ...
                    this.$api.post('/member-reversion/communalFacilities/saveSecurityFacilitiesInfo', data).then(response => {
