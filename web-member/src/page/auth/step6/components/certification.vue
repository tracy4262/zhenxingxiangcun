<template>
  <div >
    <div class="certification pd20 mb20 pt30" v-for="(item, index) in data">
      <!-- <Form :label-width="80" label-position="left" :ref="`datas${index}`" :model="item" :rules="dataLine"> -->
          ...
          <Col span="8">
            <FormItem label="会员类别" prop="member_class">
              <Cascader
                ...
                <!-- <Input v-model="item.member_class" icon="md-arrow-dropdown" readonly :disabled="!item.isEdit" placeholder="请选择"/> -->
              </Cascader>
            </FormItem>
          </Col>
          <Col span="8">
    ...
    <!-- <Button type="primary" ghost icon="md-add" class="mt20 btn-light-primary" @click="handleAdd">添加</Button> -->
  </div>
</template>
<script>
    ...
    data () {
      ...
      return {
        ...
        data: [{
          ...
          status: true,
          member_class: ['f413d522b310458aa0bf763012913a0e'],
          member_name: '',
          member_name_pinyin: '',
          member_abbreviation: '',
          abbreviation_pinyin: '',
      ...
      handleInit (e) {
        this.$api.post('/member-reversion/user/realCertification/findMemberAptitude', {
          ...
          if (response.code === 200) {
            ...
            this.data = data
            // 0是自己创建的账号 1是代理生成的账号
            this.isProxy = this.data[0].isProxy
            this.$nextTick(e => {
              for(var i = 0 ;i < data.length ; i++){
