        ...
        <div v-else>
           <Row :gutter="8" v-if="checkedData[0]">
            ...
            <div class="pd20" style="border:1px solid #e8e8e8" ref="orderRight" :style="{'min-height': orderRightHeight}">
              ...
              <Row :gutter="8" class="pb10" type="flex" align="middle" v-if="checkedData[0].contact[0] && checkedData[0].contact[0].contactNameStatus">
                <Col span="5" class="lable"><b>联系人：</b></Col>
                <Col span="19">{{checkedData[0].contact[0].contact_name}}</Col>
              </Row>
              <Row :gutter="8" class="pb10" type="flex" align="middle" v-if="checkedData[0].contact[0] && checkedData[0].contact[0].phoneStatus">
                <Col span="5"  class="lable"><b>联系电话：</b></Col>
                <Col span="19">{{checkedData[0].contact[0].phone}}</Col>
              </Row>
              <Row :gutter="8" class="pb10" type="flex" align="middle">
                <Col span="5"  class="lable"><b>用餐时间：</b></Col>
                <Col span="19">{{checkedData[0].diningTime}}</Col>
              </Row>
              <Row :gutter="8" class="pb10" type="flex" align="middle">
              ...
              <Row :gutter="8" class="pb10" type="flex" align="middle">
                ...
                <Col span="19" v-if="checkedData[0] && checkedData[0].selectedRoom && checkedData[0].selectedRoom[0]">{{checkedData[0].selectedRoom[0].management_id}}</Col>
              </Row>
              <Row :gutter="8" class="pb10" type="flex" align="middle">
                <Col span="5"  class="lable"><b>用餐人数：</b></Col>
                <Col span="19" v-if="checkedData[0] && checkedData[0].diningNumber">{{checkedData[0].diningNumber}}</Col>
              </Row>
              <Row :gutter="8" class="pb10" type="flex" align="middle">
            ...
            render: (h, params) => {
              return h(expandRow, {
                ...
                on: {
                  ...
                  'on-diningNumber-change': (e) => {
                    params.row.diningNumber = e
                    this.data.splice(params.index, 1, params.row)
                  }
                }
              })
      ...
      handleOk () {
        ...
        this.data.forEach(element => {
          if (element.checked) {
            if (element.date) {
              ...
              this.$Message.error('请选择就餐日期')
              flag = false
            }
            console.log(element.diningNumber)
            if (!element.diningNumber || element.diningNumber < 0) {
        ...
        if (flag) {
          if (!this.checkedData.length) {
            ...
            // this.title = '订单详情：'
            let data = {
              contactName: this.checkedData[0].contact && this.checkedData[0].contact[0] ? this.checkedData[0].contact[0].contact_name : '',
              phone: this.checkedData[0].contact && this.checkedData[0].contact[0] ? this.checkedData[0].contact[0].phone : '',
              account: this.checkedData[0].account,
              sellAccount: this.account,
              roomId: this.checkedData[0].selectedRoom && this.checkedData[0].selectedRoom[0] ?  this.checkedData[0].selectedRoom[0].management_id : '0',
              date: this.moment(this.checkedData[0].date).format('YYYY-MM-DD'),
              diningNumber: this.checkedData[0].diningNumber,
              time: this.checkedData[0].diningTime,
            ...
            this.$emit('on-save', {data: data, checkedData: this.checkedData})
        ...
        this.$refs['info'].validate((v) => {
          if (v) {
            ...
            let data = {
              ...
              time: this.checkedData[0].diningTime,
              setMealId: this.checkedData[0].setMealId,
              setMealName: this.checkedData[0].setMealName,
              setMeal: this.checkedData,
              checkType: '1', // 0 在线自定义套餐 1 选择固定套餐
              foodData: [],
              type: '3',
              serviceId: this.id,
              discountPrice: this.checkedData[0].setMealPrice,
