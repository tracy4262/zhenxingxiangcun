    ...
    <div class="add-service">
        <Form ref="data" :model="data" :label-width="120"   label-position="left" :rules="ruleInline">
            <!-- <FormItem label="服务时间" prop="service_times">
                <DatePicker type="daterange" v-model="data.service_times" :editable="false" style="width: 100%;" @on-change="timeChange"></DatePicker>
            </FormItem>            -->
            <FormItem label="注意事项" prop="mattres_need_attention">
                ...
                <!-- <Input  v-model="data.mattres_need_attention" /> -->
            </FormItem>
            <FormItem label="承诺内容" prop="promise_content">
                <Input type="textarea" v-model="data.promise_content" :autosize="{minRows: 3,maxRows: 5}" />
            </FormItem>
            </Form>
        <div class="tc pt20">
...
<script>
    export default {
        ...
        methods: {
                    ...
                    if (valid) {
                        this.$api.post('/member/fishing/updateFishingService', this.data).then(response => {
                            if (response.code === 200) {
                                ...
                                this.$router.push({
                                    path: '/restaurantAddService/step5',
                                    query: {
            ...
            // 上一步
            handleBack () {
                this.$router.push('/restaurantAddService/step3?id=' + this.data.id)
            },
        }
    }
</script>