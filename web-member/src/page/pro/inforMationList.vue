<template>
    <div>
        ...
        <search @searchInMa="searchInMa" :option="filterOpt" />
        <div class="layouts">
            <Row>
                <Col span="19" >
...
export default {
    ...
    methods: {
        ...
        getPolicyData(){
            let that = this;
            api.get('/member/policy/newpolicy')
            .then(res => {
                if(res.code === 200){
        ...
        getKnowledgeData(){
            let that = this;
            api.get('/member/knowLege/newknowledge')
            .then(res => {
                if(res.code === 200){
        ...
        searchGet(currentPage){
            ...
            api.get('/member/inforMation/findInforMationTitle/'+ currentPage + '?district='+ this.isObj.regionDatas + '&industry=' + this.isObj.industryDatas + '&type=' + this.isObj.informationDatas + '&goodname=' + this.isObj.productDatas + '&servicename=' + this.isObj.serviceDatas + '&species=' + this.isObj.speciesDatas + '&leixin=' + this.isObj.fileType + '&title=' + this.isObj.keywrod + '&docType=' + this.isObj.docType)
            .then(res => {
                if(res.code === 200){
                    this.isShow = true
...
<style lang="scss">
    .ma_infor_h{display: block;border-left: 4px solid #00c587;height: 14px;line-height: 14px;padding-left: 10px;}
    .news-img {
        text-align: right;
    }
    .new-last li {
