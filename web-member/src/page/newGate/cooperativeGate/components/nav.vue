<template>
    <div>
        ...
        <div class="nav-gate-cooperative" :class="active == '0/index' ? 'nav-gate-position' : ''">
          <div class="layouts">
...
export default {
    ...
    data () {
        return {
            ...
            data:[{
                title: '首页',
                url: '/cooperativePortal/index',
                checked: true,
                isShow: true
            },{
                title: '乡村介绍',
                url: '/cooperativePortal/Introduction',
                checked: false,
                isShow: true
            }, {
                title: '乡村动态',
                url: '/cooperativePortal/dynamic',
                checked: false,
                isShow: true
            }, {
                title: '乡村政策',
                url: '/cooperativePortal/policy',
                checked: false,
                isShow: true
            }, {
                title: '乡村知识',
                url: '/cooperativePortal/knowledge',
                checked: false,
                isShow: true
            }, {
                title: '标准',
                url: '/cooperativePortal/standard',
                checked: false,
                isShow: true
            }, {
                title: '会员产品',
                url: '/cooperativePortal/product',
                checked: false,
                isShow: true
            }, {
                title: '会员服务',
                url: '/cooperativePortal/service',
                checked: false,
                isShow: true
            }, {
                title: '基地',
                url: '/cooperativePortal/productionBase',
                checked: false,
                isShow: true
            }, 
            {
                title: '联系我们',
                url: '/cooperativePortal/contact',
                checked: false,
                isShow: true
            }],
            loginAccount:'',
    ...
    methods:{
        ...
        clickTab (attributionId) {
            ...
            if (attributionId.split('/').length > 2) {
                this.$router.push(`/cooperativePortal/${attributionId.split('/')[1]}?uid=${this.$route.query.uid}&tabType=${attributionId.split('/')[2]}&id=${attributionId.split('/')[0]}`)
            } else {
                this.$router.push(`/cooperativePortal/${attributionId.split('/')[1]}?uid=${this.$route.query.uid}&id=${attributionId.split('/')[0]}`)
            }
        },
        toIntroduction () {
            ...
            this.data.forEach((e, index) => {
                if (e.attributionId == `${index}/${this.active}`) {
                    ...
                    this.$router.push(`/cooperativePortal/introduction?uid=${this.loginAccount}&id=${index}`)
                }
            })
        },
        useqrcode () {
...
.nav-gate-cooperative.nav-gate-position{
    margin-top: -50px;
    background: $background;
    opacity: 0.9;
}
