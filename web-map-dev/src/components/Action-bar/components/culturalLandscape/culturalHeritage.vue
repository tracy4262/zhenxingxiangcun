<template>
  <div :class="view ? 'pd12' : 'pd20'">
    <Form ref="formItem" :model="data" :label-position="view ? 'left' : 'top'"  :rules="view ? {} : formItemInline">
      <Form-item prop="WHYCMC" label="文化遗产名称">
          <span v-if="view"> {{ data.WHYCMC }} </span>
          <Input v-model="data.WHYCMC" v-else/>
      </Form-item>
      ...
      <Form-item label="票价（元）" prop="PJ">
        <span v-if="view"> {{ data.PJ }} </span>
        <Input v-model="data.PJ" v-else>
          <span slot="append">元</span>
        </Input>
      </Form-item>
      <Form-item label="联系人" prop="JSR">
        <span v-if="view"> {{ data.JSR }} </span>
        <Input v-model="data.JSR"  v-else/>
      </Form-item>
