<template>
  <div :class="view ? 'pd12' : 'pd20'">
    <Form ref="formItem" :model="data" :label-position="view ? 'left' : 'top'"  :rules="view ? {} : formItemInline">
      ...
      <Form-item label="用户数" prop="YHS">
        <span v-if="view"> {{ data.YHS }} </span>
        <Input v-else v-model="data.YHS" :maxlength="20"/>
      </Form-item>
      <!-- <Form-item label="所处位置" prop="SCWZ">
        <span v-if="view"> {{ data.SCWZ }} </span>
        <vui-cascander v-else :values="data.SCWZ" @handle-get-result="handleGetData">
        </vui-cascander>
      </Form-item> -->
