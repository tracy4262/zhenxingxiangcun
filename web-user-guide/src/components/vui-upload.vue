<template>
  <div class="vui-upload-component">
    <Upload ref="upload" :show-upload-list="false"
      ...
      <Button type="default" :disabled="disabled"><Icon type="ios-cloud-upload-outline" size="16" class="pr5"></Icon> 上传图片</Button>
    </Upload>
    <Button type="default" :disabled="disabled" @click="handlePhotoSelectorModal('photo')"><Icon type="ios-download-outline" size="16" class="pr5"></Icon> 从文件管理导入</Button>
      ...
      <div class="new-upload-item" v-for="(item,index) in pictureList" :key="index" :style="{width: `${size[0]}px`, height: `${size[1]}px`}">
        <template v-if="item.status === 'finished'">
          ...
          <div class="upload-list-cover">
            <Icon type="ios-trash-outline" size="30" @click.native="handleRemove(item)" v-if="!disabled"></Icon>
          </div>
        </template>
        <template v-else>
