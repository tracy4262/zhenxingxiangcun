<template>
  <Form :label-width="150" label-position="left" class="mt20 mb20">
    <Form-item  label="添加关注" v-if="!listData.length">
      <Button type="primary" @click="addFollowModal = true"> <Icon type="md-add"></Icon>增加</Button>
    </Form-item>
    <Card v-for="(item, index) in listData" :key="index" class="mb10">
      <Row>
        ...
        <Col span="2" class="tr">
          <Button type="text" size="small" @click="addFollowModal = true"> <Icon type="ios-create" size="16" class="pr5"></Icon>修改</Button>
          <Button type="text" size="small" @click="handleDelSel(index)" class="mt10"><Icon type="ios-trash" size="16" class="pr5"></Icon> 删除</Button>
        </Col>
      </Row>
