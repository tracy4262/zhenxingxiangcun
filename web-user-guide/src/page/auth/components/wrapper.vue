      ...
      <div class="bg-white">
        <div class="layouts pd30">
          <Breadcrumb class="mb40">
            ...
            <BreadcrumbItem>用户向导</BreadcrumbItem>
          </Breadcrumb>
...
export default {
  ...
  methods: {
    ...
    handleStepClick (index) {
      if (this.type >= 5) {
        if (!this.current) { // 说明在第一步
          window.location.href = `${window.location.origin}/user-auth-admin/auth/step${index}`
          this.current = index - 1
        } {
          this.$router.push(`/auth/step${index}`)
          this.current = index - 1
        }
      }
    }
