  ...
  <div style="background:#f9f9f9;" class="mb40">
   <!-- 第一产业 -->
        <div class="pd20" >
          ...
          <Form :label-width="90" label-position="left" :ref="`data${index}`" v-for="(item, index) in data" :model="item":rules="dataLine">
            <Row  :gutter="16">
              <Col span="8">
                <FormItem label="地块编码">
                  <!-- <Input v-model="item.number" readonly :disabled="!edit"></Input> -->
                  <Select v-model="item.number" style="width:100" :disabled="!edit">
...
import {isNumber, isMoney3} from '~utils/validate'
  export default {
    ...
    methods: {
      // 取初始化下拉列表的数据
      handleSelectList () {
        this.$api.post('/member-reversion/landUse/dict', {type : this.type}).then(response => {
      ...
      handleSave () {
        ...
        if (flag) {
          ...
          this.$api.post('/member-reversion/landInfo/saveLandUse', list).then(response => {
              if (response.code === 200) {
