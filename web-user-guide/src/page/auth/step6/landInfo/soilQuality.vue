...
<script>
    ...
    export default {
        ...
        methods: {
            // 初始化加载数据
            init () {
                this.$api.post('/member-reversion/landInfo/findSoilQuality',{
            ...
            depictChange (item, index) {
                ...
                if (item.cried) {
                    str += `滴滴涕总量为${item.sicriedx}mg/kg，`
                }
                if (item.benzene) {
            ...
            handleSave (item, index) {
                this.$refs[`formItem${index}`][0].validate(v => {
                    if (!v) {
                        ...
                        item.dictId = this.id
                        this.$api.post('/member-reversion/landInfo/updateSoilQuality', item).then(response => {
                            if (response.code === 200) {
                                this.$Message.success('保存成功')
                                this.$emit('on-save')
                            }
