<template>
    <div class="pd20">
        ...
        <div class="pd20">
            <Form :label-width="82" label-position="left" style="background:#f9f9f9;" class="pd20 mt40" :model="item" :rules="ruleInline" v-for="(item, index) in data" @click.native="handleGetIndex(index)" :ref="`data${index}`">
                <Row :gutter="16">
                    <Col span="8">
                    <Form-item label="权限">
                ...
                <Row :gutter="16">
                    ...
                    <Form-item prop="userName" label="土地使用权利人名称">
                        <!-- 单选 -->
                        <Input v-model="item.userName" :disabled="!item.edit" />
                    </Form-item>
                    </Col>
                    <Col span="8">
                    <Form-item prop="parcelCoding" label="地块编码">
                        <Input v-model="item.parcelCoding" :maxlength="20" :disabled="!item.edit"></Input>
                    </Form-item>
                    </Col>
                    <Col span="8">
                    <Form-item prop="parcelName" label="地块名称">
                        <Input v-model="item.parcelName" :maxlength="50" :disabled="!item.edit"></Input>
                    </Form-item>
                    </Col>
                </Row>
                <Row :gutter="16">
                    ...
                    <Form-item prop="landUseStatus" label="土地利用现状" @click.natcive="">
                        ...
                        <!-- <Select v-model="item.landUseStatus" style="width:100%" :disabled="!item.edit">
                            <Option v-for="item in landUseStatuss" :value="item.label" :key="item.label">{{ item.label }}</Option>
                        </Select> -->
                        <!-- <Input v-model="item.landUseStatus" :disabled="!item.edit"></Input>        -->
                    </Form-item>
                    </Col>
                    <Col span="8">
                    <Form-item prop="aeasuredArea" label="实测面积">
                        <Input v-model="item.aeasuredArea" :maxlength="20" :disabled="!item.edit">
                        <span slot="append">平方米</span>
                        </Input>
                    </Form-item>
                    </Col>
                    <Col span="8">
                    <Form-item prop="aerialSurveyArea" label="航测面积">
                ...
                <Row :gutter="16">
                    <Col span="8">
                    <Form-item prop="contractArea" label="合同面积">
                    ...
                    <Form-item prop="getType" label="取得方式">
                        <Select v-model="item.getType" style="width: 100%" :disabled="!item.edit">
                            <Option v-for="(item, index) in getTypes" :value="item.label" :key="index">{{item.label}}</Option>
                        </Select>
                    </Form-item>
                    </Col>
                    <Col span="8">
                    <Form-item prop="getTime" label="取得时间">
                ...
                <Row :gutter="16">
                    ...
                    <Form-item prop="getPrice" label="取得价格">
                        <Input v-model="item.getPrice" :maxlength="20" :disabled="!item.edit"></Input>
                    </Form-item>
                    </Col>
                    <Col span="8">
                    <Form-item prop="userType" label="使用类型">
                    ...
                    <Form-item prop="IDName" label="证件名称">
                        <Input v-model="item.IDName" :maxlength="20" :disabled="!item.edit"></Input>
                    </Form-item>
                    </Col>
                </Row>
                <Row :gutter="16">
                    ...
                    <Form-item prop="IDNumber" label="证件号码">
                        <Input v-model="item.IDNumber" :maxlength="20" :disabled="!item.edit"></Input>
                    </Form-item>
                    </Col>
                    <Col span="8">
                    <Form-item prop="ownerName" label="发包方名称">
                        <Input v-model="item.ownerName" :maxlength="20" :disabled="!item.edit"></Input>
                    </Form-item>
                    </Col>
                    <Col span="8">
                    <Form-item prop="ownerCode" label="发包方编码">
                        <Input v-model="item.ownerCode" :maxlength="20" :disabled="!item.edit"></Input>
                    </Form-item>
                    </Col>
                </Row>
                <Row :gutter="16">
                    ...
                    <Form-item prop="ownerConcatName" label="发包方联系人姓名">
                        <Input v-model="item.ownerConcatName" :maxlength="20" :disabled="!item.edit"></Input>
                    </Form-item>
                    </Col>
                    <Col span="8">
                    <Form-item prop="ownerPhone" label="发包方联系电话">
                        <Input v-model="item.ownerPhone" :maxlength="20" :disabled="!item.edit"></Input>
                    </Form-item>
                    </Col>
                    <Col span="8">
                    <Form-item prop="ownerPostCode" label="发包方邮政编码">
                        <Input v-model="item.ownerPostCode" :maxlength="20" :disabled="!item.edit"></Input>
                    </Form-item>
                    </Col>
                </Row>
                <Row :gutter="16">
                    ...
                    <Form-item prop="group" label="">
                        <Input v-model="item.group" :maxlength="20" :disabled="!item.edit" style="width: 80px;"></Input>
                        <span class="pl10">组</span>
                    </Form-item>
                    </Col>
                    <Col span="4" class="group-number">
                    <Form-item prop="number" label="">
                        <Input v-model="item.number" :maxlength="20" :disabled="!item.edit" style="width: 80px;"></Input>
               ...
               <Col span="24">
                  <Form-item prop="ownerAddress" label="号">
                    ...
                    <Form-item prop="introduction" label="介绍">
                        <Input type="textarea" v-model="item.introduction" :maxlength="200" :autosize="{minRows: 3,maxRows: 5}" :disabled="!item.edit"></Input>
                    </Form-item>
                    </Col>
                    <Col span="24">
                    <Form-item prop="introduction" label="上传权属资料">
                        <vui-upload :ref="`upload${index}`" @on-getPictureList="getPictureList($event, index)" :total="3" :disabled="!item.edit" :multiple="false" :hint="'支持拓展名称：png jpg'" :size="[80,80]"></vui-upload>
                    </Form-item>
                    </Col>
                </Row>
...
export default {
    ...
    methods: {
        ...
        handleSelectList() {
            // 农用地
            this.$api.post('/member-reversion/landUse/dict', { type: '1' }).then(response => {
                if (response.code === 200) {
                    this.landUseStatuss[0].children = response.data
                }
            })
            // 建设用地
            this.$api.post('/member-reversion/landUse/dict', { type: '2' }).then(response => {
                if (response.code === 200) {
                    this.landUseStatuss[1].children = response.data
                }
            })
            // 未利用地
            this.$api.post('/member-reversion/landUse/dict', { type: '3' }).then(response => {
                if (response.code === 200) {
            ...
            this.$api.post('/member-reversion/assetSeting/findLandUseRightInfo', {
                ...
                if (response.code == 200) {
                    ...
                    if (!response.data.textPreview.textPreview) {
                        response.data.textPreview.textPreview = `土地使用权利人名称（），地块编码（），地块名称（），土地利用现状（），实测面积（）平方米，航测面积（）平方米，合同面积（）平方米，取得方式（），取得时间（），取得价格（），使用类型（），证件名称（），证件号码（），发包方名称（），发包方编码（），发包方联系人姓名（），发包方联系电话（），发包方邮政编码（），发包方地址（），（）组，（）号，介绍（）。`
                    }
